...
jur_CIBackReferenceSet_getName = $this => {
    ...
    jl_StringBuilder_append1(jl_StringBuilder_append(var$2, $rt_s(9)), var$1);
...
igiaj_EngineJs_activationOrder = $manifests => {
    ...
    while ($index < $manifests.length) {
        ...
        $id = igiaj_EngineJs_text($tree, $rt_s(10));
        if ($id !== null) {
            $ids.$add2($id);
            $services = igiaj_EngineJs_member($tree, $rt_s(11));
            $provides.$put($id, igiaj_EngineJs_strings(igiaj_EngineJs_member($services, $rt_s(12))));
            $consumes.$put($id, igiaj_EngineJs_strings(igiaj_EngineJs_member($services, $rt_s(13))));
        }
        $index = $index + 1 | 0;
    }
...
igiaj_EngineJs_booleanOrNull = $value => {
    igiaj_EngineJs_$callClinit();
    if ($value !== null && !(igiaj_EngineJs_nullish$js_body$_11($value) ? 1 : 0))
        return jl_Boolean_valueOf(!!$value ? 1 : 0);
    return null;
},
igiaj_EngineJs_nullish$js_body$_11 = var$1 => {
    return var$1 === null || var$1 === undefined;
},
igiaj_EngineJs_plan$js_body$_13 = (var$1, var$2) => {
...
igiaj_EngineJs_isPluginError$exported$3 = var$1 => {
    igiaj_EngineJs_$callClinit();
    return !!igiaj_EngineJs_isPluginError(var$1);
},
igiaj_EngineJs_setStrict$exported$4 = var$1 => {
    igiaj_EngineJs_$callClinit();
    igiaj_EngineJs_setStrict(var$1);
},
igiaj_EngineJs_setDiagnosticSink$exported$5 = var$1 => {
    igiaj_EngineJs_$callClinit();
...
jur_CISequenceSet_getName = $this => {
    ...
    var$1 = $this.$string0;
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append(var$2, $rt_s(14)), var$1);
    return jl_StringBuilder_toString(var$2);
};
...
igiae_Activation_order = ($ids, $provides, $consumes) => {
    let $providersOf, var$5, $id, var$7, $serviceId, $owners, $dependencies, $needed, var$12, $provider, $cycles, $settled, $cycle, $order, $progressed, $pending, $dependency;
    $providersOf = ju_HashMap__init_();
    var$5 = $ids.$iterator();
    while (var$5.$hasNext()) {
        $id = var$5.$next();
        var$7 = (igiae_Activation_listOf($provides, $id)).$iterator();
        while (var$7.$hasNext()) {
            $serviceId = var$7.$next();
            $owners = $providersOf.$get1($serviceId);
            if ($owners === null) {
                $owners = ju_ArrayList__init_();
                $providersOf.$put($serviceId, $owners);
            }
            $owners.$add2($id);
        }
    }
    $dependencies = ju_HashMap__init_();
    var$7 = $ids.$iterator();
    while (var$7.$hasNext()) {
        $id = var$7.$next();
        $needed = ju_LinkedHashSet__init_0();
        var$12 = (igiae_Activation_listOf($consumes, $id)).$iterator();
        while (var$12.$hasNext()) {
            $serviceId = var$12.$next();
            var$5 = (igiae_Activation_listOf($providersOf, $serviceId)).$iterator();
            while (var$5.$hasNext()) {
                $provider = var$5.$next();
                if (!jl_String_equals($provider, $id))
                    $needed.$add2($provider);
            }
        }
        $dependencies.$put($id, $needed);
    }
    $cycles = igiae_Activation_findCycles($ids, $dependencies);
    ...
    while ($progressed) {
        ...
        while (var$5.$hasNext()) {
            $id = var$5.$next();
            if ($settled.$contains0($id))
                continue;
            $pending = 0;
            var$7 = ($dependencies.$get1($id)).$iterator();
            a: {
                while (var$7.$hasNext()) {
                    $dependency = var$7.$next();
                    if (!$settled.$contains0($dependency)) {
                        $pending = 1;
                        break a;
                    }
                }
...
jur_UCIBackReferenceSet_getName = $this => {
    ...
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append1(jl_StringBuilder_append(var$2, $rt_s(17)), var$1);
    return jl_StringBuilder_toString(var$2);
};
...
jur_CharClass$15_contains = ($this, $ch) => {
    return $this.$val$clazz9.$contains($ch) && $this.$val$curAlt1 ^ $this.$this$06.$bits.$get0($ch) ? 1 : 0;
};
...
jl_AbstractStringBuilder_insert3 = ($this, $index, $obj) => {
    return $this.$insert0($index, $obj === null ? $rt_s(22) : $obj.$toString());
},
...
let jl_StringBuilder_insert9 = ($this, $index, $string) => {
    jl_AbstractStringBuilder_insert2($this, $index, $string);
    return $this;
},
...
jur_CompositeRangeSet_getName = $this => {
    ...
    var$3 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append(jl_StringBuilder_append(jl_StringBuilder_append(var$3, $rt_s(23)), var$1), $rt_s(24)), var$2);
    return jl_StringBuilder_toString(var$3);
},
...
igiae_Diagnostics_isStrict = () => {
    ...
    if (igiae_Diagnostics_strict === null)
        igiae_Diagnostics_strict = jl_Boolean_valueOf(jl_String_equals($rt_s(25), jl_System_getenv($rt_s(26))));
...
igiae_Diagnostics_ignoreUnknown = ($kind, $id, $source) => {
    igiae_Diagnostics_$callClinit();
    igiae_Diagnostics_report((((((((jl_StringBuilder__init_()).$append13($rt_s(27))).$append13($kind)).$append13($rt_s(28))).$append13($id)).$append13($rt_s(29))).$append13($source)).$toString());
},
igiae_Diagnostics_report = $message => {
    ...
    if (igiae_Diagnostics_isStrict())
        (jl_System_err()).$println((((jl_StringBuilder__init_()).$append13($rt_s(30))).$append13($message)).$toString());
},
igiae_Diagnostics__clinit_ = () => {
    igiae_Diagnostics_strict = null;
    igiae_Diagnostics_sink = null;
...
jur_LowHighSurrogateRangeSet_getName = $this => {
    let var$1, var$2, var$3;
    var$1 = !$this.$alt1 ? $rt_s(31) : $rt_s(32);
    var$2 = $this.$surrChars.$toString();
    var$3 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append(jl_StringBuilder_append(var$3, $rt_s(33)), var$1), var$2);
    return jl_StringBuilder_toString(var$3);
},
                ...
                try {
                    while ($it.$hasNext()) {
                        ...
                        if (!$other.$containsKey($entry.$getKey()))
                            break b;
                        if (!ju_Objects_equals($entry.$getValue0(), $other.$get1($entry.$getKey())))
                            break c;
                    }
                } catch ($$e) {
                    $$je = $rt_wrapException($$e);
...
ju_AbstractMap_toString = $this => {
    ...
    if ($iter.$hasNext()) {
        $e = $iter.$next();
        $sb.$append($e.$getKey() !== $this ? $e.$getKey() : $rt_s(35));
        $sb.$append0(61);
        $sb.$append($e.$getValue0() !== $this ? $e.$getValue0() : $rt_s(35));
    }
    while ($iter.$hasNext()) {
        $sb.$append13($rt_s(36));
        $e = $iter.$next();
...
jur_AbstractCharClass$PredefinedCharacterClasses_getObject = ($this, $name) => {
    ...
    while (true) {
        ...
        if ($i >= jur_AbstractCharClass$PredefinedCharacterClasses_contents.data.length)
            $rt_throw(ju_MissingResourceException__init_0($rt_s(38), $rt_s(38), $name));
...
jur_DecomposedCharSet_getName = $this => {
    ...
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append(var$2, $rt_s(233)), var$1);
    return jl_StringBuilder_toString(var$2);
},
...
igiae_Activation$Tarjan_visit = ($this, $id) => {
    ...
    a: {
        $this.$index0.$put($id, jl_Integer_valueOf($this.$counter));
        ...
        if ($needed !== null) {
            ...
            while (true) {
                ...
                if (!$this.$index0.$containsKey($dependency)) {
                ...
                } else if ($this.$onStack.$contains0($dependency))
                    $this.$lowLink.$put($id, jl_Integer_valueOf(jl_Math_min(($this.$lowLink.$get1($id)).$intValue(), ($this.$index0.$get1($dependency)).$intValue())));
            }
        }
    }
    if (!($this.$lowLink.$get1($id)).$equals($this.$index0.$get1($id)))
        return;
    $component = ju_ArrayList__init_();
    while (true) {
        $member = $this.$stack.$pop();
        $this.$onStack.$remove0($member);
...
jur_Quantifier_toString = $this => {
    let var$1, var$2, var$3;
    var$1 = $this.$min1;
    var$2 = $this.$max1 == 2147483647 ? $rt_s(38) : jl_Integer_toString($this.$max1);
    var$3 = jl_StringBuilder__init_();
...
let igiae_PluginException__init_ = ($this, $pluginId, $detail, $fix) => {
    jl_RuntimeException__init_0($this, (((((((jl_StringBuilder__init_()).$append13($rt_s(238))).$append13($pluginId)).$append13($rt_s(239))).$append13($detail)).$append13($rt_s(240))).$append13($fix)).$toString());
...
igiaj_JsErrors_mint = ($pluginId, $detail, $fix) => {
    ...
    jl_StringBuilder_append(jl_StringBuilder_append(jl_StringBuilder_append(jl_StringBuilder_append(jl_StringBuilder_append(jl_StringBuilder_append0(var$4, 91), $pluginId), $rt_s(239)), $detail), $rt_s(240)), $fix);
...
jur_AltQuantifierSet__init_ = ($this, var$1, var$2, var$3) => {
    jur_LeafQuantifierSet__init_($this, var$1, var$2, var$3);
},
...
igiae_ManifestValidator_validate = ($value, $wellKnownServices) => {
    ...
    $source = igiae_ManifestValidator_stringAt($value, $rt_s(10));
    if (!($source !== null && !jl_String_isEmpty($source)))
        $source = $rt_s(243);
    $structural = igiae_SchemaValidator_validate($value, igiae_ManifestSchema_get(), $rt_s(244), $source);
    ...
    $issues.$addAll(igiae_ManifestValidator_duplicateIssues($rt_s(245), igiae_ManifestValidator_listAt($value, $rt_s(245))));
    $issues.$addAll(igiae_ManifestValidator_duplicateIssues($rt_s(246), igiae_ManifestValidator_listAt(igiae_ManifestValidator_mapAt($value, $rt_s(11)), $rt_s(12))));
    $issues.$addAll(igiae_ManifestValidator_duplicateIssues($rt_s(247), igiae_ManifestValidator_listAt(igiae_ManifestValidator_mapAt($value, $rt_s(11)), $rt_s(13))));
    $issues.$addAll(igiae_ManifestValidator_duplicateIssues($rt_s(248), igiae_ManifestValidator_listAt($value, $rt_s(248))));
    return $issues;
},
igiae_ManifestValidator_require = ($value, $wellKnownServices) => {
    ...
    if ($issues.$isEmpty())
        return $value;
    $pluginId = igiae_ManifestValidator_stringAt($value, $rt_s(10));
    if (!($pluginId !== null && !jl_String_isEmpty($pluginId)))
        $pluginId = $rt_s(249);
    $first = $issues.$get(0);
    $rt_throw(igiae_PluginException__init_0($pluginId, (((((jl_StringBuilder__init_()).$append13($rt_s(250))).$append13(igiae_SchemaIssue_getPath($first))).$append13($rt_s(4))).$append13(igiae_SchemaIssue_getMessage($first))).$toString(), igiae_SchemaIssue_getFix($first)));
},
igiae_ManifestValidator_entryIssues = $value => {
    let $issues, $capabilities, $entry;
    $issues = ju_ArrayList__init_();
    $capabilities = igiae_ManifestValidator_listAt($value, $rt_s(245));
    $entry = igiae_ManifestValidator_stringAt($value, $rt_s(251));
    if ($capabilities !== null && !$capabilities.$isEmpty() && !($entry !== null && !jl_String_isEmpty($entry))) {
        $issues.$add2(igiae_SchemaIssue__init_0($rt_s(251), $rt_s(252), $rt_s(253)));
        return $issues;
    }
    if ($entry !== null && !jl_String_isEmpty($entry) && igiae_ManifestValidator_escapesRepo($entry))
        $issues.$add2(igiae_SchemaIssue__init_0($rt_s(251), ((((jl_StringBuilder__init_()).$append13($rt_s(254))).$append13($entry)).$append13($rt_s(255))).$toString(), $rt_s(256)));
    return $issues;
},
igiae_ManifestValidator_escapesRepo = $entry => {
    let var$2, var$3, var$4, $segment;
    if (!jl_String_startsWith($entry, $rt_s(257)) && !jl_String_startsWith($entry, $rt_s(258))) {
        if (jl_String_length($entry) >= 2 && jl_String_charAt($entry, 1) == 58 && jl_Character_isLetter0(jl_String_charAt($entry, 0)))
            return 1;
        var$2 = (jl_String_split($entry, $rt_s(259))).data;
        var$3 = var$2.length;
        var$4 = 0;
        while (var$4 < var$3) {
            $segment = var$2[var$4];
            if (jl_String_equals($rt_s(260), $segment))
                return 1;
            var$4 = var$4 + 1 | 0;
        }
        return 0;
...
igiae_ManifestValidator_providedServiceIssues = ($value, $wellKnownServices) => {
    ...
    $issues = ju_ArrayList__init_();
    $pluginId = igiae_ManifestValidator_stringAt($value, $rt_s(10));
    $provides = igiae_ManifestValidator_listAt(igiae_ManifestValidator_mapAt($value, $rt_s(11)), $rt_s(12));
    if ($provides === null)
        return $issues;
    $index = 0;
    while ($index < $provides.$size()) {
        ...
        if (!igiae_ManifestValidator_mayRegister($pluginId, $serviceId, $wellKnownServices)) {
            $tail = !jl_String_contains($serviceId, $rt_s(261)) ? $serviceId : jl_String_substring0($serviceId, jl_String_lastIndexOf1($serviceId, 58) + 1 | 0);
            var$9 = new igiae_SchemaIssue;
            var$10 = ((((jl_StringBuilder__init_()).$append13($rt_s(262))).$append2($index)).$append13($rt_s(263))).$toString();
            var$11 = ((((jl_StringBuilder__init_()).$append13($rt_s(254))).$append13($serviceId)).$append13($rt_s(264))).$toString();
            var$12 = (((((jl_StringBuilder__init_()).$append13($rt_s(265))).$append13($pluginId)).$append13($rt_s(261))).$append13($tail)).$append13($rt_s(266));
            igiae_SchemaIssue__init_(var$9, var$10, var$11, (var$12.$append13(igiae_ManifestValidator_join($wellKnownServices))).$toString());
            $issues.$add2(var$9);
        }
        $index = $index + 1 | 0;
    }
...
igiae_ManifestValidator_mayRegister = ($pluginId, $serviceId, $wellKnownServices) => {
    let $separator;
    if ($wellKnownServices !== null && $wellKnownServices.$contains0($serviceId))
        return 1;
...
igiae_ManifestValidator_duplicateIssues = ($path, $values) => {
    ...
    while ($index < $values.$size()) {
        ...
        if (!$seen.$add2($value))
            $issues.$add2(igiae_SchemaIssue__init_0((((((jl_StringBuilder__init_()).$append13($path)).$append13($rt_s(238))).$append2($index)).$append13($rt_s(263))).$toString(), ((((jl_StringBuilder__init_()).$append13($rt_s(254))).$append13($value)).$append13($rt_s(267))).$toString(), $rt_s(268)));
        $index = $index + 1 | 0;
    }
    return $issues;
},
...
igiae_ManifestValidator_join = $values => {
    ...
    while ($index < $values.$size()) {
        if ($index > 0)
            $out.$append13($rt_s(36));
        $out.$append13($values.$get($index));
        $index = $index + 1 | 0;
    }
    return $out.$toString();
},
...
ju_Objects_requireNonNull = $obj => {
    return ju_Objects_requireNonNull0($obj, $rt_s(38));
},
...
jur_SequenceSet_accepts = ($this, $strIndex, $testString) => {
    return !$this.$startsWith0($testString, $strIndex) ? (-1) : $this.$charCount0;
},
...
jur_SequenceSet_getName = $this => {
    ...
    var$1 = $this.$string;
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append(var$2, $rt_s(270)), var$1);
    return jl_StringBuilder_toString(var$2);
},
jur_SequenceSet_first = ($this, $set) => {
    ...
    b: {
        c: {
            var$2 = $set;
            if (!var$2.$contains(jl_String_charAt($this.$string, 0))) {
                var$6 = $this.$string;
...
jur_UCIRangeSet_getName = $this => {
    let var$1, var$2, var$3;
    var$1 = !$this.$alt3 ? $rt_s(31) : $rt_s(32);
    var$2 = $this.$chars1.$toString();
    var$3 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append(jl_StringBuilder_append(var$3, $rt_s(272)), var$1), var$2);
    return jl_StringBuilder_toString(var$3);
};
...
jl_Boolean_toString0 = $value => {
    jl_Boolean_$callClinit();
    return !$value ? $rt_s(274) : $rt_s(275);
},
jl_Boolean_toString = $this => {
    return jl_Boolean_toString0($this.$value2);
},
jl_Boolean_hashCode = $this => {
    return jl_Boolean_hashCode0($this.$value2);
},
...
jur_BackReferenceSet_getName = $this => {
    ...
    var$1 = $this.$groupIndex;
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append1(jl_StringBuilder_append(var$2, $rt_s(278)), var$1);
    return jl_StringBuilder_toString(var$2);
},
            ...
            if ($itemType !== null) {
                ...
                if ($itemName !== null) {
                    if ($itemType[$rt_meta].itemType !== null) {
                        ...
                        var$5 = jl_StringBuilder__init_();
                        jl_StringBuilder_append0(jl_StringBuilder_append(jl_StringBuilder_append(var$5, $rt_s(281)), $itemName), 59);
                        $result = jl_StringBuilder_toString(var$5);
                    }
                }
            }
...
jur_UCISupplCharSet_getName = $this => {
    ...
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append(var$2, $rt_s(283)), var$1);
    return jl_StringBuilder_toString(var$2);
},
...
jl_System_arraycopy = ($src, $srcPos, $dest, $destPos, $length) => {
    ...
    $rt_throw(jl_NullPointerException__init_($rt_s(284)));
},
...
igiaj_JsJson_toTree = $items => {
    ...
    if ($items !== null && !(igiaj_JsJson_isNullish$js_body$_4($items) ? 1 : 0)) {
...
igiaj_JsJson_isNullish$js_body$_4 = var$1 => {
    return var$1 === null || var$1 === undefined;
},
...
jur_CharClass$9_contains = ($this, $ch) => {
    return !$this.$val$clazz.$contains($ch) && !($this.$val$curAlt8 ^ $this.$this$05.$bits.$get0($ch)) ? 0 : 1;
},
...
jur_CICharSet_getName = $this => {
    ...
    var$1 = $this.$ch3;
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append0(jl_StringBuilder_append(var$2, $rt_s(290)), var$1);
    return jl_StringBuilder_toString(var$2);
};
...
jur_SupplRangeSet_getName = $this => {
    let var$1, var$2, var$3;
    var$1 = !$this.$alt0 ? $rt_s(31) : $rt_s(32);
...
jur_HangulDecomposedCharSet_getName = $this => {
    ...
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append(var$2, $rt_s(291)), var$1);
    return jl_StringBuilder_toString(var$2);
},
...
igiae_SchemaIssue_toString = $this => {
    return (((((((jl_StringBuilder__init_()).$append13($this.$path)).$append13($rt_s(4))).$append13($this.$message)).$append13($rt_s(292))).$append13($this.$fix)).$append13($rt_s(293))).$toString();
},
...
ju_LinkedHashMapIterator_checkConcurrentMod = $this => {
    if ($this.$expectedModCount0 == $this.$base.$modCount)
        return;
    $rt_throw(ju_ConcurrentModificationException__init_());
},
ju_LinkedHashMapIterator_makeNext = $this => {
    ...
    $this.$futureEntry0 = !$this.$reversed1 ? $this.$futureEntry0.$chainForward : $this.$futureEntry0.$chainBackward;
};
...
ju_LinkedHashMapKeySet_iterator = $this => {
    return ju_LinkedHashMapIterator$KeyIterator__init_0($this.$base0, $this.$reversed0);
};
...
igiae_SchemaValidator_checkObject = ($value, $schema, $path, $issues, $source) => {
    ...
    a: {
        ...
        if ($required !== null) {
            ...
            while (true) {
                ...
                var$13 = ((((jl_StringBuilder__init_()).$append13($rt_s(299))).$append13($key)).$append13($rt_s(300))).$toString();
                igiae_SchemaIssue__init_(var$11, var$12, var$13, $child !== null && igiae_JsonSchema_getFix($child) !== null ? igiae_JsonSchema_getFix($child) : (((((jl_StringBuilder__init_()).$append13($rt_s(301))).$append13($key)).$append13($rt_s(302))).$append13($path)).$toString());
                $issues.$add2(var$11);
            }
        }
    }
    ...
    while (var$8.$hasNext()) {
        ...
        if ($childSchema !== null) {
            igiae_SchemaValidator_check($entry.$getValue0(), $childSchema, igiae_SchemaValidator_childPath($path, $key), $issues, $source);
            continue;
        }
        if ($source === null)
            continue;
        igiae_Diagnostics_ignoreUnknown($rt_s(303), igiae_SchemaValidator_childPath($path, $key), $source);
    }
},
...
igiae_SchemaValidator_checkArray = ($value, $schema, $path, $issues, $source) => {
    ...
    $index = 0;
    while ($index < $values.$size()) {
        igiae_SchemaValidator_check($values.$get($index), $items, (((((jl_StringBuilder__init_()).$append13($path)).$append13($rt_s(238))).$append2($index)).$append13($rt_s(263))).$toString(), $issues, $source);
        $index = $index + 1 | 0;
    }
},
igiae_SchemaValidator_checkString = ($value, $schema, $path, $issues) => {
    ...
    if ($pattern !== null && !jur_Matcher_find0(jur_Pattern_matcher(jur_Pattern_compile($pattern), $text))) {
        ...
        var$8 = (((((jl_StringBuilder__init_()).$append13($rt_s(254))).$append13($text)).$append13($rt_s(304))).$append13($pattern)).$toString();
        igiae_SchemaIssue__init_(var$7, $path, var$8, igiae_JsonSchema_getFix($schema) !== null ? igiae_JsonSchema_getFix($schema) : (((((jl_StringBuilder__init_()).$append13($rt_s(305))).$append13($path)).$append13($rt_s(306))).$append13($pattern)).$toString());
        $issues.$add2(var$7);
    }
    $allowed = igiae_JsonSchema_getEnumValues($schema);
    if ($allowed !== null && !$allowed.$contains0($text)) {
        $joined = igiae_SchemaValidator_join($allowed);
        var$11 = new igiae_SchemaIssue;
        var$7 = (((((jl_StringBuilder__init_()).$append13($rt_s(254))).$append13($text)).$append13($rt_s(307))).$append13($joined)).$toString();
        igiae_SchemaIssue__init_(var$11, $path, var$7, igiae_JsonSchema_getFix($schema) !== null ? igiae_JsonSchema_getFix($schema) : (((((jl_StringBuilder__init_()).$append13($rt_s(308))).$append13($path)).$append13($rt_s(309))).$append13($joined)).$toString());
        $issues.$add2(var$11);
    }
},
igiae_SchemaValidator_checkNumber = ($value, $schema, $path, $issues) => {
    ...
    a: {
        if ($value instanceof jl_Number) {
            ...
            if (!igiae_SchemaValidator_isNotANumber(var$5)) {
                if (!jl_String_equals($rt_s(298), igiae_JsonSchema_getType($schema)))
                    break a;
    ...
    if ($minimum !== null && var$5.$doubleValue() < $minimum.$doubleValue()) {
        var$7 = new igiae_SchemaIssue;
        var$8 = (((jl_StringBuilder__init_()).$append13($rt_s(310))).$append($minimum)).$append13($rt_s(311));
        var$8 = (var$8.$append13(igiae_SchemaValidator_describeNumber(var$5))).$toString();
        igiae_SchemaIssue__init_(var$7, $path, var$8, igiae_JsonSchema_getFix($schema) !== null ? igiae_JsonSchema_getFix($schema) : (((((jl_StringBuilder__init_()).$append13($rt_s(308))).$append13($path)).$append13($rt_s(312))).$append($minimum)).$toString());
        $issues.$add2(var$7);
    }
},
...
igiae_SchemaValidator_typeIssue = ($value, $schema, $path) => {
    ...
    var$5 = (((((jl_StringBuilder__init_()).$append13($rt_s(313))).$append13(igiae_JsonSchema_getType($schema))).$append13($rt_s(311))).$append13(igiae_SchemaValidator_describe($value))).$toString();
    igiae_SchemaIssue__init_(var$4, $path, var$5, igiae_JsonSchema_getFix($schema) !== null ? igiae_JsonSchema_getFix($schema) : (((((jl_StringBuilder__init_()).$append13($rt_s(308))).$append13($path)).$append13($rt_s(314))).$append13(igiae_JsonSchema_getType($schema))).$toString());
    return var$4;
},
...
igiae_SchemaValidator_childPath = ($path, $key) => {
    if (!jl_String_equals($rt_s(244), $path))
        $key = ((((jl_StringBuilder__init_()).$append13($path)).$append13($rt_s(288))).$append13($key)).$toString();
    return $key;
};
...
ju_HashMap$1_iterator = $this => {
    return ju_HashMap$KeyIterator__init_0($this.$this$04);
};
...
jur_UCICharSet_getName = $this => {
    ...
    var$1 = $this.$ch2;
    var$2 = jl_StringBuilder__init_();
    jl_StringBuilder_append0(jl_StringBuilder_append(var$2, $rt_s(283)), var$1);
    return jl_StringBuilder_toString(var$2);
};
function jl_Double() {
    jl_Number.call(this);
...
jl_Double_toString = $d => {
    jl_Double_$callClinit();
    return ((jl_StringBuilder__init_()).$append15($d)).$toString();
},
...
ju_ArrayList_clear = $this => {
    ju_Arrays_fill2($this.$array, 0, $this.$size0, null);
    $this.$size0 = 0;
    $this.$modCount0 = $this.$modCount0 + 1 | 0;
},
ju_ArrayList_checkIndex = ($this, $index) => {
...
ju_ArrayList_toString = $this => {
    ...
    $i = 0;
    while ($i < $length) {
        ($buffer.$append($this.$array.data[$i] === $this ? $rt_s(241) : $this.$array.data[$i])).$append13($rt_s(36));
        $i = $i + 1 | 0;
    }
    $buffer.$append($this.$array.data[$length] === $this ? $rt_s(241) : $this.$array.data[$length]);
...
jl_String_valueOf = $obj => {
    jl_String_$callClinit();
    return $obj === null ? $rt_s(22) : $obj.$toString();
},
...
jl_String_valueOf1 = $l => {
    jl_String_$callClinit();
    return ((jl_StringBuilder__init_()).$append16($l)).$toString();
},
...
ju_LinkedHashSet__init_ = $this => {
    ju_HashSet__init_($this, ju_LinkedHashMap__init_());
},
ju_LinkedHashSet__init_0 = () => {
    let var_0 = new ju_LinkedHashSet();
    ju_LinkedHashSet__init_(var_0);
    return var_0;
},
...
ju_LinkedHashMapEntrySet_iterator = $this => {
    return ju_LinkedHashMapIterator$EntryIterator__init_0($this.$base1, $this.$reversed);
},
...
jur_Pattern_compile0 = ($pattern, $flags) => {
    if ($pattern === null)
        $rt_throw(jl_NullPointerException__init_($rt_s(320)));
    if ($flags && ($flags | 255) != 255)
        $rt_throw(jl_IllegalArgumentException__init_2($rt_s(38)));
    jur_AbstractSet_$callClinit();
    jur_AbstractSet_counter = 1;
...
jur_Pattern_compileImpl = ($this, $pattern, $flags) => {
    ...
    $rt_throw(jur_PatternSyntaxException__init_($rt_s(38), $this.$lexemes.$toString(), $this.$lexemes.$getIndex()));
},
...
jur_Pattern_processExpression = ($this, $ch, $newFlags, $last) => {
    ...
    while (true) {
        if ($this.$lexemes.$isLetter1() && $this.$lexemes.$lookAhead() == (-536870788))
...
jur_Pattern_processDecomposedChar = $this => {
    ...
    $curSymb = (-1);
    $curSymbIndex = (-1);
    if (!$this.$lexemes.$isEmpty() && $this.$lexemes.$isLetter1()) {
    ...
    var$10 = 1;
    while (var$10 < 4 && !$this.$lexemes.$isEmpty() && $this.$lexemes.$isLetter1()) {
        var$5 = $codePoints.data;
        var$9 = var$10 + 1 | 0;
...
jur_Pattern_processSubExpression = ($this, $last) => {
    ...
    if ($this.$lexemes.$isLetter1() && !$this.$lexemes.$isNextSpecial() && jur_Lexer_isLetter($this.$lexemes.$lookAhead())) {
        if (!jur_Pattern_hasFlag($this, 128)) {
            ...
            if (!$this.$lexemes.$isEmpty()) {
                ...
                if (!(var$4.$peek() == (-536870871) && !($last instanceof jur_FinalSet))) {
                    var$4 = $this.$lexemes;
                    if (var$4.$peek() != (-536870788) && !$this.$lexemes.$isLetter1())
...
jur_Pattern_processRangeExpression = ($this, $alt) => {
    ...
    a: {
        b: {
            ...
            $rt_throw(jur_PatternSyntaxException__init_($rt_s(38), jur_Pattern_pattern($this), $this.$lexemes.$getIndex()));
        }
        $rt_throw(jur_PatternSyntaxException__init_($rt_s(38), jur_Pattern_pattern($this), $this.$lexemes.$getIndex()));
    }
    if (!$notClosed) {
    ...
    $rt_throw(jur_PatternSyntaxException__init_($rt_s(38), jur_Pattern_pattern($this), $this.$lexemes.$getIndex() - 1 | 0));
},
...
jur_Pattern_quote = $s => {
    ...
    $sb = (jl_StringBuilder__init_()).$append13($rt_s(321));
    $apos = 0;
    while (true) {
        var$4 = jl_String_indexOf0($s, $rt_s(322), $apos);
        if (var$4 < 0)
            break;
        $apos_0 = var$4 + 2 | 0;
        ($sb.$append13(jl_String_substring($s, $apos, $apos_0))).$append13($rt_s(323));
        $apos = $apos_0;
    }
    return (($sb.$append13(jl_String_substring0($s, $apos))).$append13($rt_s(322))).$toString();
},
            ...
            b: {
                if ($this.$lookAhead0 == 92) {
                    ...
                    c: {
                        ...
                        switch ($this.$lookAhead0) {
                            case -1:
                                $rt_throw(jur_PatternSyntaxException__init_($rt_s(38), $this.$toString(), $this.$index));
                            case 0:
                            case 1:
                            case 2:
                            case 3:
                                ...
                                d: {
                                    if (!$behindOrNamed) {
                                        ...
                                        if ($nonCap > 57)
                                            break a;
                                        if ($nameBuilder === null) {
                                            var$8 = jur_PatternSyntaxException__init_($rt_s(325), $this.$toString(), $this.$index);
                                            $rt_throw(var$8);
                                        }
                                        $nameBuilder.$append0($nonCap);
                                        jur_Lexer_nextIndex($this);
...
jur_Lexer_parseCharClassName = $this => {
    ...
    if ($this.$index < ($this.$pattern0.data.length - 2 | 0)) {
        if ($this.$pattern0.data[$this.$index] != 123) {
            ...
            var$3 = jl_StringBuilder__init_();
            jl_StringBuilder_append(jl_StringBuilder_append(var$3, $rt_s(326)), var$2);
            return jl_StringBuilder_toString(var$3);
        }
        jur_Lexer_nextIndex($this);
        $ch = 0;
    ...
    if (jl_String_length($res) == 1) {
        var$2 = jl_StringBuilder__init_();
        jl_StringBuilder_append(jl_StringBuilder_append(var$2, $rt_s(326)), $res);
        return jl_StringBuilder_toString(var$2);
    }
    b: {
        c: {
            if (jl_String_length($res) > 3) {
                if (jl_String_startsWith($res, $rt_s(326)))
                    break c;
                if (jl_String_startsWith($res, $rt_s(327)))
                    break c;
            }
            break b;
        }
...
igiae_ManifestSchema_get = () => {
    ...
    $properties.$put($rt_s(328), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(285)), $rt_s(329)));
    $id = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(285)), $rt_s(330));
    igiae_JsonSchema_setPattern($id, $rt_s(331));
    igiae_JsonSchema_setFix($id, $rt_s(332));
    $properties.$put($rt_s(10), $id);
    $api = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(298)), $rt_s(333));
    igiae_JsonSchema_setMinimum($api, jl_Integer_valueOf(1));
    igiae_JsonSchema_setFix($api, $rt_s(334));
    $properties.$put($rt_s(335), $api);
    $entry = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(285)), $rt_s(336));
    igiae_JsonSchema_setFix($entry, $rt_s(337));
    $properties.$put($rt_s(251), $entry);
    $properties.$put($rt_s(338), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(285)), $rt_s(339)));
    $properties.$put($rt_s(340), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(285)), $rt_s(341)));
    $capabilities = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(297)), $rt_s(342));
    igiae_JsonSchema_setItems($capabilities, igiae_JsonSchema_ofType($rt_s(285)));
    igiae_JsonSchema_setFix($capabilities, $rt_s(343));
    $properties.$put($rt_s(245), $capabilities);
    $properties.$put($rt_s(11), igiae_ManifestSchema_services());
    $permissions = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(297)), $rt_s(344));
    igiae_JsonSchema_setItems($permissions, igiae_JsonSchema_ofType($rt_s(285)));
    $properties.$put($rt_s(248), $permissions);
    $properties.$put($rt_s(345), igiae_ManifestSchema_lifecycle());
    $properties.$put($rt_s(346), igiae_ManifestSchema_publish());
    $properties.$put($rt_s(347), igiae_ManifestSchema_repo());
    $root = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(296)), $rt_s(348));
    igiae_JsonSchema_setTitle($root, $rt_s(349));
    igiae_JsonSchema_setRequired($root, ju_Arrays_asList($rt_wrapArray(jl_String, [$rt_s(10), $rt_s(335)])));
    igiae_JsonSchema_setProperties($root, $properties);
    return $root;
},
igiae_ManifestSchema_services = () => {
    let $properties, $provides, $consumes, $services;
    $properties = ju_LinkedHashMap__init_();
    $provides = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(297)), $rt_s(350));
    igiae_JsonSchema_setItems($provides, igiae_JsonSchema_ofType($rt_s(285)));
    $properties.$put($rt_s(12), $provides);
    $consumes = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(297)), $rt_s(351));
    igiae_JsonSchema_setItems($consumes, igiae_JsonSchema_ofType($rt_s(285)));
    $properties.$put($rt_s(13), $consumes);
    $services = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(296)), $rt_s(352));
...
igiae_ManifestSchema_lifecycle = () => {
    ...
    $properties = ju_LinkedHashMap__init_();
    $properties.$put($rt_s(353), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(287)), $rt_s(354)));
    $properties.$put($rt_s(355), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(287)), $rt_s(356)));
    $lifecycle = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(296)), $rt_s(357));
...
igiae_ManifestSchema_publish = () => {
    ...
    $properties = ju_LinkedHashMap__init_();
    $properties.$put($rt_s(358), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(287)), $rt_s(359)));
    $publish = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(296)), $rt_s(360));
...
igiae_ManifestSchema_repo = () => {
    ...
    $properties = ju_LinkedHashMap__init_();
    $properties.$put($rt_s(361), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(285)), $rt_s(362)));
    $properties.$put($rt_s(363), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(285)), $rt_s(364)));
    $domains = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(297)), $rt_s(365));
    igiae_JsonSchema_setItems($domains, igiae_JsonSchema_ofType($rt_s(285)));
    $properties.$put($rt_s(366), $domains);
    $properties.$put($rt_s(367), igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(285)), $rt_s(368)));
    $repo = igiae_ManifestSchema_described(igiae_JsonSchema_ofType($rt_s(296)), $rt_s(369));
    igiae_JsonSchema_setRequired($repo, ju_Arrays_asList($rt_wrapArray(jl_String, [$rt_s(361), $rt_s(363), $rt_s(367)])));
...
jur_RangeSet_getName = $this => {
    let var$1, var$2, var$3;
    var$1 = !$this.$alt2 ? $rt_s(31) : $rt_s(32);
...
let ju_Arrays$ArrayAsList__init_ = (var$0, var$1) => {
    ju_AbstractList__init_(var$0);
    var$0.$array1 = var$1;
},
...
$rt_packages([-1, "java", 0, "util", 1, "regex", 0, "lang", -1, "org", 4, "teavm", 5, "classlib", 6, "impl", 7, "unicode", -1, "io", 9, "github", 10, "intisy", 11, "ai", 12, "engine"
]);
...
jl_IndexOutOfBoundsException, 0, jl_RuntimeException, [], 1, 0, 0, ["$_init_", $rt_wrapFunction0(jl_IndexOutOfBoundsException__init_0), "$_init_0", $rt_wrapFunction1(jl_IndexOutOfBoundsException__init_2)],
...
jur_AbstractCharClass$LazyJavaWhitespace$1, "AbstractCharClass$LazyJavaWhitespace$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_47", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaWhitespace$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaWhitespace$1_contains)],
...
jl_Integer, "Integer", 3, jl_Number, [jl_Comparable], 1, 0, () => jl_Integer_$callClinit(), ["$_init_1", $rt_wrapFunction1(jl_Integer__init_), "$intValue", $rt_wrapFunction0(jl_Integer_intValue), "$doubleValue", $rt_wrapFunction0(jl_Integer_doubleValue), "$toString", $rt_wrapFunction0(jl_Integer_toString1), "$hashCode0", $rt_wrapFunction0(jl_Integer_hashCode), "$equals", $rt_wrapFunction1(jl_Integer_equals)],
jl_AbstractStringBuilder$Constants, 0, jl_Object, [], 0, 0, () => jl_AbstractStringBuilder$Constants_$callClinit(), 0,
jur_AbstractSet, 0, jl_Object, [], 1024, 0, () => jur_AbstractSet_$callClinit(), ["$_init_", $rt_wrapFunction0(jur_AbstractSet__init_), "$_init_9", $rt_wrapFunction1(jur_AbstractSet__init_0), "$find", $rt_wrapFunction3(jur_AbstractSet_find), "$findBack", $rt_wrapFunction4(jur_AbstractSet_findBack), "$setType", $rt_wrapFunction1(jur_AbstractSet_setType), "$getType", $rt_wrapFunction0(jur_AbstractSet_getType), "$getQualifiedName", $rt_wrapFunction0(jur_AbstractSet_getQualifiedName), "$toString", $rt_wrapFunction0(jur_AbstractSet_toString),
...
jur_JointSet, "JointSet", 2, jur_AbstractSet, [], 0, 0, 0, ["$_init_", $rt_wrapFunction0(jur_JointSet__init_), "$_init_27", $rt_wrapFunction2(jur_JointSet__init_0), "$matches", $rt_wrapFunction3(jur_JointSet_matches), "$setNext", $rt_wrapFunction1(jur_JointSet_setNext), "$getName", $rt_wrapFunction0(jur_JointSet_getName), "$first", $rt_wrapFunction1(jur_JointSet_first), "$hasConsumed", $rt_wrapFunction1(jur_JointSet_hasConsumed), "$processSecondPass", $rt_wrapFunction0(jur_JointSet_processSecondPass)],
jur_SingleSet, "SingleSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_6", $rt_wrapFunction2(jur_SingleSet__init_), "$matches", $rt_wrapFunction3(jur_SingleSet_matches), "$find", $rt_wrapFunction3(jur_SingleSet_find), "$findBack", $rt_wrapFunction4(jur_SingleSet_findBack), "$first", $rt_wrapFunction1(jur_SingleSet_first), "$processBackRefReplacement", $rt_wrapFunction0(jur_SingleSet_processBackRefReplacement), "$processSecondPass", $rt_wrapFunction0(jur_SingleSet_processSecondPass)],
...
jur_SequenceSet$IntHash, 0, jl_Object, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_SequenceSet$IntHash__init_0), "$put0", $rt_wrapFunction2(jur_SequenceSet$IntHash_put), "$get3", $rt_wrapFunction1(jur_SequenceSet$IntHash_get)],
...
jur_BackReferencedSingleSet, "BackReferencedSingleSet", 2, jur_SingleSet, [], 0, 0, 0, ["$_init_5", $rt_wrapFunction1(jur_BackReferencedSingleSet__init_), "$find", $rt_wrapFunction3(jur_BackReferencedSingleSet_find), "$findBack", $rt_wrapFunction4(jur_BackReferencedSingleSet_findBack), "$processBackRefReplacement", $rt_wrapFunction0(jur_BackReferencedSingleSet_processBackRefReplacement)],
jur_CIBackReferenceSet, "CIBackReferenceSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_15", $rt_wrapFunction2(jur_CIBackReferenceSet__init_), "$matches", $rt_wrapFunction3(jur_CIBackReferenceSet_matches), "$setNext", $rt_wrapFunction1(jur_CIBackReferenceSet_setNext), "$getString", $rt_wrapFunction1(jur_CIBackReferenceSet_getString), "$getName", $rt_wrapFunction0(jur_CIBackReferenceSet_getName), "$hasConsumed", $rt_wrapFunction1(jur_CIBackReferenceSet_hasConsumed)],
...
jur_AbstractCharClass$1, "AbstractCharClass$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_2", $rt_wrapFunction2(jur_AbstractCharClass$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$1_contains)],
igiaj_EngineJs, 0, jl_Object, [], 17, 0, () => igiaj_EngineJs_$callClinit(), 0,
jur_AbstractCharClass$2, "AbstractCharClass$2", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_3", $rt_wrapFunction3(jur_AbstractCharClass$2__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$2_contains)],
...
jur_LeafSet, 0, jur_AbstractSet, [], 1024, 0, 0, ["$_init_9", $rt_wrapFunction1(jur_LeafSet__init_0), "$_init_", $rt_wrapFunction0(jur_LeafSet__init_), "$matches", $rt_wrapFunction3(jur_LeafSet_matches), "$charCount", $rt_wrapFunction0(jur_LeafSet_charCount), "$hasConsumed", $rt_wrapFunction1(jur_LeafSet_hasConsumed)],
jur_CISequenceSet, "CISequenceSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_56", $rt_wrapFunction1(jur_CISequenceSet__init_), "$accepts", $rt_wrapFunction2(jur_CISequenceSet_accepts), "$getName", $rt_wrapFunction0(jur_CISequenceSet_getName)],
jur_QuantifierSet, 0, jur_AbstractSet, [], 1024, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_QuantifierSet__init_), "$getInnerSet", $rt_wrapFunction0(jur_QuantifierSet_getInnerSet), "$first", $rt_wrapFunction1(jur_QuantifierSet_first), "$hasConsumed", $rt_wrapFunction1(jur_QuantifierSet_hasConsumed), "$processSecondPass", $rt_wrapFunction0(jur_QuantifierSet_processSecondPass)],
jur_LeafQuantifierSet, "LeafQuantifierSet", 2, jur_QuantifierSet, [], 0, 0, 0, ["$_init_11", $rt_wrapFunction3(jur_LeafQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_LeafQuantifierSet_matches), "$getName", $rt_wrapFunction0(jur_LeafQuantifierSet_getName)],
jur_CompositeQuantifierSet, "CompositeQuantifierSet", 2, jur_LeafQuantifierSet, [], 0, 0, 0, ["$_init_12", $rt_wrapFunction4(jur_CompositeQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_CompositeQuantifierSet_matches), "$getName", $rt_wrapFunction0(jur_CompositeQuantifierSet_getName)],
jur_PossessiveCompositeQuantifierSet, "PossessiveCompositeQuantifierSet", 2, jur_CompositeQuantifierSet, [], 0, 0, 0, ["$_init_12", $rt_wrapFunction4(jur_PossessiveCompositeQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_PossessiveCompositeQuantifierSet_matches)],
...
ju_MissingResourceException, 0, jl_RuntimeException, [], 1, 0, 0, ["$_init_21", $rt_wrapFunction3(ju_MissingResourceException__init_)],
jur_AbstractCharClass$LazyJavaLetterOrDigit$1, "AbstractCharClass$LazyJavaLetterOrDigit$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_65", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaLetterOrDigit$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaLetterOrDigit$1_contains)],
jur_CharClass$18, "CharClass$18", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_71", $rt_wrapFunction2(jur_CharClass$18__init_), "$contains", $rt_wrapFunction1(jur_CharClass$18_contains), "$toString", $rt_wrapFunction0(jur_CharClass$18_toString)],
jur_GroupQuantifierSet, "GroupQuantifierSet", 2, jur_QuantifierSet, [], 0, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_GroupQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_GroupQuantifierSet_matches), "$getName", $rt_wrapFunction0(jur_GroupQuantifierSet_getName)]]);
$rt_metadata([jur_PossessiveGroupQuantifierSet, "PossessiveGroupQuantifierSet", 2, jur_GroupQuantifierSet, [], 0, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_PossessiveGroupQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_PossessiveGroupQuantifierSet_matches)],
igiae_Activation, 0, jl_Object, [], 17, 0, 0, 0,
jur_UCIBackReferenceSet, "UCIBackReferenceSet", 2, jur_CIBackReferenceSet, [], 0, 0, 0, ["$_init_15", $rt_wrapFunction2(jur_UCIBackReferenceSet__init_), "$matches", $rt_wrapFunction3(jur_UCIBackReferenceSet_matches), "$getName", $rt_wrapFunction0(jur_UCIBackReferenceSet_getName)],
jur_CharClass$13, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_68", $rt_wrapFunction2(jur_CharClass$13__init_), "$contains", $rt_wrapFunction1(jur_CharClass$13_contains)],
jur_CharClass$12, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_68", $rt_wrapFunction2(jur_CharClass$12__init_), "$contains", $rt_wrapFunction1(jur_CharClass$12_contains)],
jur_CharClass$11, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_67", $rt_wrapFunction4(jur_CharClass$11__init_), "$contains", $rt_wrapFunction1(jur_CharClass$11_contains)],
jur_AbstractCharClass$LazyCategory, 0, jur_AbstractCharClass$LazyCharClass, [], 0, 0, 0, ["$_init_22", $rt_wrapFunction2(jur_AbstractCharClass$LazyCategory__init_0), "$_init_23", $rt_wrapFunction3(jur_AbstractCharClass$LazyCategory__init_1), "$computeValue", $rt_wrapFunction0(jur_AbstractCharClass$LazyCategory_computeValue)],
otci_Base46, 0, jl_Object, [], 17, 0, 0, 0,
jur_CharClass$10, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_67", $rt_wrapFunction4(jur_CharClass$10__init_), "$contains", $rt_wrapFunction1(jur_CharClass$10_contains)],
jur_CharClass$17, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_67", $rt_wrapFunction4(jur_CharClass$17__init_), "$contains", $rt_wrapFunction1(jur_CharClass$17_contains)],
jur_UCISequenceSet, "UCISequenceSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_56", $rt_wrapFunction1(jur_UCISequenceSet__init_), "$accepts", $rt_wrapFunction2(jur_UCISequenceSet_accepts), "$getName", $rt_wrapFunction0(jur_UCISequenceSet_getName)],
jur_CharClass$16, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_67", $rt_wrapFunction4(jur_CharClass$16__init_), "$contains", $rt_wrapFunction1(jur_CharClass$16_contains)],
jur_DotAllQuantifierSet, "DotAllQuantifierSet", 2, jur_QuantifierSet, [], 0, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_DotAllQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_DotAllQuantifierSet_matches), "$find", $rt_wrapFunction3(jur_DotAllQuantifierSet_find), "$getName", $rt_wrapFunction0(jur_DotAllQuantifierSet_getName)],
jur_CharClass$15, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_70", $rt_wrapFunction3(jur_CharClass$15__init_), "$contains", $rt_wrapFunction1(jur_CharClass$15_contains)],
jur_AbstractCharClass$LazyJavaDefined$1, "AbstractCharClass$LazyJavaDefined$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_51", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaDefined$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaDefined$1_contains)],
jur_CharClass$14, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_70", $rt_wrapFunction3(jur_CharClass$14__init_), "$contains", $rt_wrapFunction1(jur_CharClass$14_contains)],
...
jur_FSet, "FSet", 2, jur_AbstractSet, [], 0, 0, () => jur_FSet_$callClinit(), ["$_init_1", $rt_wrapFunction1(jur_FSet__init_), "$matches", $rt_wrapFunction3(jur_FSet_matches), "$getGroupIndex", $rt_wrapFunction0(jur_FSet_getGroupIndex), "$getName", $rt_wrapFunction0(jur_FSet_getName), "$hasConsumed", $rt_wrapFunction1(jur_FSet_hasConsumed)],
jur_BehindFSet, "BehindFSet", 2, jur_FSet, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_BehindFSet__init_), "$matches", $rt_wrapFunction3(jur_BehindFSet_matches), "$getName", $rt_wrapFunction0(jur_BehindFSet_getName)],
jl_AbstractStringBuilder, 0, jl_Object, [ji_Serializable, jl_CharSequence], 0, 0, 0, ["$_init_", $rt_wrapFunction0(jl_AbstractStringBuilder__init_0), "$_init_1", $rt_wrapFunction1(jl_AbstractStringBuilder__init_), "$append5", $rt_wrapFunction1(jl_AbstractStringBuilder_append6), "$append6", $rt_wrapFunction1(jl_AbstractStringBuilder_append2), "$insert0", $rt_wrapFunction2(jl_AbstractStringBuilder_insert2), "$append7", $rt_wrapFunction1(jl_AbstractStringBuilder_append3), "$append1", $rt_wrapFunction2(jl_AbstractStringBuilder_append7),
"$insert1", $rt_wrapFunction3(jl_AbstractStringBuilder_insert6), "$append8", $rt_wrapFunction1(jl_AbstractStringBuilder_append5), "$insert2", $rt_wrapFunction2(jl_AbstractStringBuilder_insert4), "$insert3", $rt_wrapFunction3(jl_AbstractStringBuilder_insert5), "$append9", $rt_wrapFunction1(jl_AbstractStringBuilder_append4), "$insert4", $rt_wrapFunction2(jl_AbstractStringBuilder_insert1), "$append10", $rt_wrapFunction1(jl_AbstractStringBuilder_append1), "$insert5", $rt_wrapFunction2(jl_AbstractStringBuilder_insert0),
"$insert", $rt_wrapFunction2(jl_AbstractStringBuilder_insert3), "$ensureCapacity", $rt_wrapFunction1(jl_AbstractStringBuilder_ensureCapacity), "$toString", $rt_wrapFunction0(jl_AbstractStringBuilder_toString), "$length", $rt_wrapFunction0(jl_AbstractStringBuilder_length), "$charAt", $rt_wrapFunction1(jl_AbstractStringBuilder_charAt), "$append4", $rt_wrapFunction3(jl_AbstractStringBuilder_append0), "$insert6", $rt_wrapFunction4(jl_AbstractStringBuilder_insert), "$append11", $rt_wrapFunction1(jl_AbstractStringBuilder_append),
...
jl_StringBuilder, 0, jl_AbstractStringBuilder, [jl_Appendable], 1, 0, 0, ["$_init_1", $rt_wrapFunction1(jl_StringBuilder__init_2), "$_init_", $rt_wrapFunction0(jl_StringBuilder__init_1), "$append", $rt_wrapFunction1(jl_StringBuilder_append), "$append13", $rt_wrapFunction1(jl_StringBuilder_append2), "$append2", $rt_wrapFunction1(jl_StringBuilder_append1), "$append16", $rt_wrapFunction1(jl_StringBuilder_append7), "$append15", $rt_wrapFunction1(jl_StringBuilder_append4), "$append0", $rt_wrapFunction1(jl_StringBuilder_append0),
"$append12", $rt_wrapFunction3(jl_StringBuilder_append3), "$append3", $rt_wrapFunction1(jl_StringBuilder_append6), "$insert11", $rt_wrapFunction2(jl_StringBuilder_insert5), "$insert10", $rt_wrapFunction2(jl_StringBuilder_insert6), "$insert7", $rt_wrapFunction4(jl_StringBuilder_insert3), "$insert8", $rt_wrapFunction2(jl_StringBuilder_insert7), "$insert9", $rt_wrapFunction2(jl_StringBuilder_insert1), "$delete0", $rt_wrapFunction2(jl_StringBuilder_delete), "$deleteCharAt", $rt_wrapFunction1(jl_StringBuilder_deleteCharAt),
"$insert12", $rt_wrapFunction2(jl_StringBuilder_insert9), "$insert6", $rt_wrapFunction4(jl_StringBuilder_insert8), "$append4", $rt_wrapFunction3(jl_StringBuilder_append5), "$length", $rt_wrapFunction0(jl_StringBuilder_length), "$toString", $rt_wrapFunction0(jl_StringBuilder_toString), "$ensureCapacity", $rt_wrapFunction1(jl_StringBuilder_ensureCapacity), "$insert", $rt_wrapFunction2(jl_StringBuilder_insert0), "$insert5", $rt_wrapFunction2(jl_StringBuilder_insert), "$insert4", $rt_wrapFunction2(jl_StringBuilder_insert4),
...
ju_ConcurrentModificationException, 0, jl_RuntimeException, [], 1, 0, 0, ["$_init_", $rt_wrapFunction0(ju_ConcurrentModificationException__init_0)],
igiae_Diagnostics, 0, jl_Object, [], 17, 0, () => igiae_Diagnostics_$callClinit(), 0,
jur_LowHighSurrogateRangeSet, "LowHighSurrogateRangeSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_42", $rt_wrapFunction1(jur_LowHighSurrogateRangeSet__init_), "$setNext", $rt_wrapFunction1(jur_LowHighSurrogateRangeSet_setNext), "$matches", $rt_wrapFunction3(jur_LowHighSurrogateRangeSet_matches), "$getName", $rt_wrapFunction0(jur_LowHighSurrogateRangeSet_getName)],
jur_ReluctantGroupQuantifierSet, "ReluctantGroupQuantifierSet", 2, jur_GroupQuantifierSet, [], 0, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_ReluctantGroupQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_ReluctantGroupQuantifierSet_matches)],
...
ju_HashMap, 0, ju_AbstractMap, [jl_Cloneable, ji_Serializable], 1, 0, 0, ["$newElementArray", $rt_wrapFunction1(ju_HashMap_newElementArray), "$_init_", $rt_wrapFunction0(ju_HashMap__init_1), "$_init_1", $rt_wrapFunction1(ju_HashMap__init_0), "$_init_17", $rt_wrapFunction2(ju_HashMap__init_2), "$containsKey", $rt_wrapFunction1(ju_HashMap_containsKey), "$get1", $rt_wrapFunction1(ju_HashMap_get), "$entryByKey", $rt_wrapFunction1(ju_HashMap_entryByKey), "$findNonNullKeyEntry", $rt_wrapFunction3(ju_HashMap_findNonNullKeyEntry),
"$findNullKeyEntry", $rt_wrapFunction0(ju_HashMap_findNullKeyEntry), "$keySet", $rt_wrapFunction0(ju_HashMap_keySet), "$put", $rt_wrapFunction2(ju_HashMap_put), "$rehash0", $rt_wrapFunction1(ju_HashMap_rehash0), "$rehash", $rt_wrapFunction0(ju_HashMap_rehash), "$remove1", $rt_wrapFunction1(ju_HashMap_remove), "$removeEntry", $rt_wrapFunction1(ju_HashMap_removeEntry), "$removeByKey", $rt_wrapFunction1(ju_HashMap_removeByKey), "$size", $rt_wrapFunction0(ju_HashMap_size)],
...
"$sequencedKeySet", $rt_wrapFunction0(ju_LinkedHashMap_sequencedKeySet), "$remove1", $rt_wrapFunction1(ju_LinkedHashMap_remove), "$removeLinkedEntry", $rt_wrapFunction1(ju_LinkedHashMap_removeLinkedEntry), "$removeEldestEntry", $rt_wrapFunction1(ju_LinkedHashMap_removeEldestEntry)],
jur_PosPlusGroupQuantifierSet, "PosPlusGroupQuantifierSet", 2, jur_GroupQuantifierSet, [], 0, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_PosPlusGroupQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_PosPlusGroupQuantifierSet_matches)],
jur_EmptySet, "EmptySet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_9", $rt_wrapFunction1(jur_EmptySet__init_0), "$accepts", $rt_wrapFunction2(jur_EmptySet_accepts), "$find", $rt_wrapFunction3(jur_EmptySet_find), "$findBack", $rt_wrapFunction4(jur_EmptySet_findBack), "$getName", $rt_wrapFunction0(jur_EmptySet_getName), "$hasConsumed", $rt_wrapFunction1(jur_EmptySet_hasConsumed)],
jl_StringBuffer, 0, jl_AbstractStringBuilder, [jl_Appendable], 1, 0, 0, ["$_init_", $rt_wrapFunction0(jl_StringBuffer__init_), "$append17", $rt_wrapFunction1(jl_StringBuffer_append1), "$append14", $rt_wrapFunction3(jl_StringBuffer_append2), "$append18", $rt_wrapFunction1(jl_StringBuffer_append0), "$insert13", $rt_wrapFunction4(jl_StringBuffer_insert), "$insert14", $rt_wrapFunction2(jl_StringBuffer_insert2), "$insert6", $rt_wrapFunction4(jl_StringBuffer_insert1), "$append4", $rt_wrapFunction3(jl_StringBuffer_append),
...
jur_AbstractCharClass$LazyJavaUnicodeIdentifierPart$1, "AbstractCharClass$LazyJavaUnicodeIdentifierPart$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_50", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaUnicodeIdentifierPart$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaUnicodeIdentifierPart$1_contains)],
...
jur_DecomposedCharSet, "DecomposedCharSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_25", $rt_wrapFunction2(jur_DecomposedCharSet__init_), "$setNext", $rt_wrapFunction1(jur_DecomposedCharSet_setNext), "$matches", $rt_wrapFunction3(jur_DecomposedCharSet_matches), "$getName", $rt_wrapFunction0(jur_DecomposedCharSet_getName), "$codePointAt", $rt_wrapFunction3(jur_DecomposedCharSet_codePointAt), "$first", $rt_wrapFunction1(jur_DecomposedCharSet_first), "$hasConsumed", $rt_wrapFunction1(jur_DecomposedCharSet_hasConsumed)],
jur_CIDecomposedCharSet, "CIDecomposedCharSet", 2, jur_DecomposedCharSet, [], 0, 0, 0, ["$_init_25", $rt_wrapFunction2(jur_CIDecomposedCharSet__init_)],
ju_Map$Entry, 0, jl_Object, [], 1537, 0, 0, 0,
ju_MapEntry, 0, jl_Object, [ju_Map$Entry, jl_Cloneable], 0, 0, 0, ["$_init_26", $rt_wrapFunction2(ju_MapEntry__init_), "$getKey", $rt_wrapFunction0(ju_MapEntry_getKey), "$getValue0", $rt_wrapFunction0(ju_MapEntry_getValue), "$hashCode0", $rt_wrapFunction0(ju_MapEntry_hashCode)],
ju_HashMap$HashEntry, 0, ju_MapEntry, [], 0, 0, 0, ["$_init_19", $rt_wrapFunction2(ju_HashMap$HashEntry__init_)],
ju_LinkedHashMap$LinkedHashMapEntry, 0, ju_HashMap$HashEntry, [], 16, 0, 0, ["$_init_19", $rt_wrapFunction2(ju_LinkedHashMap$LinkedHashMapEntry__init_)],
...
$rt_metadata([jur_AbstractCharClass$LazyASCII, 0, jur_AbstractCharClass$LazyCharClass, [], 0, 0, 0, ["$_init_", $rt_wrapFunction0(jur_AbstractCharClass$LazyASCII__init_), "$computeValue", $rt_wrapFunction0(jur_AbstractCharClass$LazyASCII_computeValue)],
jur_NonCapJointSet, "NonCapJointSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_27", $rt_wrapFunction2(jur_NonCapJointSet__init_), "$matches", $rt_wrapFunction3(jur_NonCapJointSet_matches), "$getName", $rt_wrapFunction0(jur_NonCapJointSet_getName), "$hasConsumed", $rt_wrapFunction1(jur_NonCapJointSet_hasConsumed)],
jur_AtomicJointSet, "AtomicJointSet", 2, jur_NonCapJointSet, [], 0, 0, 0, ["$_init_27", $rt_wrapFunction2(jur_AtomicJointSet__init_), "$matches", $rt_wrapFunction3(jur_AtomicJointSet_matches), "$setNext", $rt_wrapFunction1(jur_AtomicJointSet_setNext), "$getName", $rt_wrapFunction0(jur_AtomicJointSet_getName)],
jur_PositiveLookAhead, "PositiveLookAhead", 2, jur_AtomicJointSet, [], 0, 0, 0, ["$_init_27", $rt_wrapFunction2(jur_PositiveLookAhead__init_), "$matches", $rt_wrapFunction3(jur_PositiveLookAhead_matches), "$hasConsumed", $rt_wrapFunction1(jur_PositiveLookAhead_hasConsumed), "$getName", $rt_wrapFunction0(jur_PositiveLookAhead_getName)],
jur_NegativeLookAhead, "NegativeLookAhead", 2, jur_AtomicJointSet, [], 0, 0, 0, ["$_init_27", $rt_wrapFunction2(jur_NegativeLookAhead__init_), "$matches", $rt_wrapFunction3(jur_NegativeLookAhead_matches), "$hasConsumed", $rt_wrapFunction1(jur_NegativeLookAhead_hasConsumed), "$getName", $rt_wrapFunction0(jur_NegativeLookAhead_getName)],
igiae_Activation$Tarjan, 0, jl_Object, [], 16, 0, 0, ["$_init_14", $rt_wrapFunction1(igiae_Activation$Tarjan__init_), "$visit", $rt_wrapFunction1(igiae_Activation$Tarjan_visit)],
ju_Iterator, 0, jl_Object, [], 1537, 0, 0, 0,
ju_AbstractList$1, 0, jl_Object, [ju_Iterator], 0, 0, 0, ["$_init_40", $rt_wrapFunction1(ju_AbstractList$1__init_), "$hasNext", $rt_wrapFunction0(ju_AbstractList$1_hasNext), "$next", $rt_wrapFunction0(ju_AbstractList$1_next)],
...
jur_Quantifier, "Quantifier", 2, jur_SpecialToken, [jl_Cloneable], 0, 0, 0, ["$_init_15", $rt_wrapFunction2(jur_Quantifier__init_), "$min", $rt_wrapFunction0(jur_Quantifier_min), "$max", $rt_wrapFunction0(jur_Quantifier_max), "$toString", $rt_wrapFunction0(jur_Quantifier_toString)],
jur_AbstractCharClass$LazyJavaUpperCase$1, "AbstractCharClass$LazyJavaUpperCase$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_43", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaUpperCase$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaUpperCase$1_contains)],
...
igiae_PluginException, 0, jl_RuntimeException, [], 1, 0, 0, ["$_init_21", $rt_wrapFunction3(igiae_PluginException__init_), "$getPluginId", $rt_wrapFunction0(igiae_PluginException_getPluginId), "$getDetail", $rt_wrapFunction0(igiae_PluginException_getDetail), "$getFix", $rt_wrapFunction0(igiae_PluginException_getFix)],
...
ju_AbstractCollection, 0, jl_Object, [ju_Collection], 1025, 0, 0, ["$_init_", $rt_wrapFunction0(ju_AbstractCollection__init_), "$isEmpty", $rt_wrapFunction0(ju_AbstractCollection_isEmpty), "$contains0", $rt_wrapFunction1(ju_AbstractCollection_contains), "$toArray", $rt_wrapFunction1(ju_AbstractCollection_toArray), "$addAll", $rt_wrapFunction1(ju_AbstractCollection_addAll), "$toString", $rt_wrapFunction0(ju_AbstractCollection_toString)],
jur_PossessiveQuantifierSet, "PossessiveQuantifierSet", 2, jur_LeafQuantifierSet, [], 0, 0, 0, ["$_init_11", $rt_wrapFunction3(jur_PossessiveQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_PossessiveQuantifierSet_matches)],
otci_IntegerUtil, 0, jl_Object, [], 17, 0, 0, 0,
igiaj_JsErrors, 0, jl_Object, [], 16, 0, 0, 0,
jur_AltQuantifierSet, "AltQuantifierSet", 2, jur_LeafQuantifierSet, [], 0, 0, 0, ["$_init_11", $rt_wrapFunction3(jur_AltQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_AltQuantifierSet_matches), "$setNext", $rt_wrapFunction1(jur_AltQuantifierSet_setNext)],
jur_PossessiveAltQuantifierSet, "PossessiveAltQuantifierSet", 2, jur_AltQuantifierSet, [], 0, 0, 0, ["$_init_11", $rt_wrapFunction3(jur_PossessiveAltQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_PossessiveAltQuantifierSet_matches)],
jur_AbstractCharClass$LazyJavaIdentifierIgnorable$1, "AbstractCharClass$LazyJavaIdentifierIgnorable$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_64", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaIdentifierIgnorable$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaIdentifierIgnorable$1_contains)],
jur_AbstractCharClass$LazyJavaLetter$1, "AbstractCharClass$LazyJavaLetter$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_24", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaLetter$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaLetter$1_contains)],
jur_ReluctantQuantifierSet, "ReluctantQuantifierSet", 2, jur_LeafQuantifierSet, [], 0, 0, 0, ["$_init_11", $rt_wrapFunction3(jur_ReluctantQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_ReluctantQuantifierSet_matches)],
otji_JS, 0, jl_Object, [], 17, 0, 0, 0,
jur_AbstractCharClass$LazyJavaUnicodeIdentifierStart$1, "AbstractCharClass$LazyJavaUnicodeIdentifierStart$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_66", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaUnicodeIdentifierStart$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaUnicodeIdentifierStart$1_contains)],
...
jur_PositiveLookBehind, "PositiveLookBehind", 2, jur_AtomicJointSet, [], 0, 0, 0, ["$_init_27", $rt_wrapFunction2(jur_PositiveLookBehind__init_), "$matches", $rt_wrapFunction3(jur_PositiveLookBehind_matches), "$hasConsumed", $rt_wrapFunction1(jur_PositiveLookBehind_hasConsumed), "$getName", $rt_wrapFunction0(jur_PositiveLookBehind_getName)],
jur_SequenceSet, "SequenceSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_56", $rt_wrapFunction1(jur_SequenceSet__init_), "$accepts", $rt_wrapFunction2(jur_SequenceSet_accepts), "$find", $rt_wrapFunction3(jur_SequenceSet_find), "$findBack", $rt_wrapFunction4(jur_SequenceSet_findBack), "$getName", $rt_wrapFunction0(jur_SequenceSet_getName), "$first", $rt_wrapFunction1(jur_SequenceSet_first), "$indexOf0", $rt_wrapFunction3(jur_SequenceSet_indexOf), "$lastIndexOf0", $rt_wrapFunction3(jur_SequenceSet_lastIndexOf), "$startsWith0",
...
jur_AltGroupQuantifierSet, "AltGroupQuantifierSet", 2, jur_GroupQuantifierSet, [], 0, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_AltGroupQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_AltGroupQuantifierSet_matches), "$setNext", $rt_wrapFunction1(jur_AltGroupQuantifierSet_setNext)],
...
ju_HashMap$AbstractMapIterator, 0, jl_Object, [], 0, 0, 0, ["$_init_18", $rt_wrapFunction1(ju_HashMap$AbstractMapIterator__init_), "$hasNext", $rt_wrapFunction0(ju_HashMap$AbstractMapIterator_hasNext), "$checkConcurrentMod", $rt_wrapFunction0(ju_HashMap$AbstractMapIterator_checkConcurrentMod), "$makeNext", $rt_wrapFunction0(ju_HashMap$AbstractMapIterator_makeNext)],
ju_HashMap$KeyIterator, 0, ju_HashMap$AbstractMapIterator, [ju_Iterator], 0, 0, 0, ["$_init_18", $rt_wrapFunction1(ju_HashMap$KeyIterator__init_), "$next", $rt_wrapFunction0(ju_HashMap$KeyIterator_next)],
jur_UCIRangeSet, "UCIRangeSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_42", $rt_wrapFunction1(jur_UCIRangeSet__init_), "$accepts", $rt_wrapFunction2(jur_UCIRangeSet_accepts), "$getName", $rt_wrapFunction0(jur_UCIRangeSet_getName)],
otji_JSWrapper, 0, jl_Object, [], 17, 0, 0, 0]);
$rt_metadata([jur_AbstractCharClass$LazyJavaJavaIdentifierPart$1, "AbstractCharClass$LazyJavaJavaIdentifierPart$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_36", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaJavaIdentifierPart$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaJavaIdentifierPart$1_contains)],
...
ju_HashSet, "HashSet", 1, ju_AbstractSet, [jl_Cloneable, ji_Serializable], 1, 0, 0, ["$_init_", $rt_wrapFunction0(ju_HashSet__init_1), "$_init_18", $rt_wrapFunction1(ju_HashSet__init_), "$add2", $rt_wrapFunction1(ju_HashSet_add), "$contains0", $rt_wrapFunction1(ju_HashSet_contains), "$iterator", $rt_wrapFunction0(ju_HashSet_iterator), "$remove0", $rt_wrapFunction1(ju_HashSet_remove), "$size", $rt_wrapFunction0(ju_HashSet_size)],
...
ji_FilterOutputStream, 0, ji_OutputStream, [], 1, 0, 0, ["$_init_33", $rt_wrapFunction1(ji_FilterOutputStream__init_)],
ji_PrintStream, 0, ji_FilterOutputStream, [jl_Appendable], 1, 0, 0, ["$_init_34", $rt_wrapFunction3(ji_PrintStream__init_)],
...
jl_Boolean, "Boolean", 3, jl_Object, [ji_Serializable, jl_Comparable], 1, 0, () => jl_Boolean_$callClinit(), ["$_init_35", $rt_wrapFunction1(jl_Boolean__init_0), "$booleanValue", $rt_wrapFunction0(jl_Boolean_booleanValue), "$toString", $rt_wrapFunction0(jl_Boolean_toString), "$hashCode0", $rt_wrapFunction0(jl_Boolean_hashCode), "$equals", $rt_wrapFunction1(jl_Boolean_equals)],
jur_MultiLineSOLSet, "MultiLineSOLSet", 2, jur_AbstractSet, [], 0, 0, 0, ["$_init_61", $rt_wrapFunction1(jur_MultiLineSOLSet__init_), "$matches", $rt_wrapFunction3(jur_MultiLineSOLSet_matches), "$hasConsumed", $rt_wrapFunction1(jur_MultiLineSOLSet_hasConsumed), "$getName", $rt_wrapFunction0(jur_MultiLineSOLSet_getName)],
ju_NoSuchElementException, 0, jl_RuntimeException, [], 1, 0, 0, ["$_init_", $rt_wrapFunction0(ju_NoSuchElementException__init_0)],
...
jur_NegativeLookBehind, "NegativeLookBehind", 2, jur_AtomicJointSet, [], 0, 0, 0, ["$_init_27", $rt_wrapFunction2(jur_NegativeLookBehind__init_), "$matches", $rt_wrapFunction3(jur_NegativeLookBehind_matches), "$hasConsumed", $rt_wrapFunction1(jur_NegativeLookBehind_hasConsumed), "$getName", $rt_wrapFunction0(jur_NegativeLookBehind_getName)],
jur_BackReferenceSet, "BackReferenceSet", 2, jur_CIBackReferenceSet, [], 0, 0, 0, ["$_init_15", $rt_wrapFunction2(jur_BackReferenceSet__init_), "$matches", $rt_wrapFunction3(jur_BackReferenceSet_matches), "$find", $rt_wrapFunction3(jur_BackReferenceSet_find), "$findBack", $rt_wrapFunction4(jur_BackReferenceSet_findBack), "$first", $rt_wrapFunction1(jur_BackReferenceSet_first), "$getName", $rt_wrapFunction0(jur_BackReferenceSet_getName)],
...
jur_DotQuantifierSet, "DotQuantifierSet", 2, jur_QuantifierSet, [], 0, 0, 0, ["$_init_60", $rt_wrapFunction4(jur_DotQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_DotQuantifierSet_matches), "$find", $rt_wrapFunction3(jur_DotQuantifierSet_find), "$getName", $rt_wrapFunction0(jur_DotQuantifierSet_getName)],
...
jur_UnifiedQuantifierSet, "UnifiedQuantifierSet", 2, jur_LeafQuantifierSet, [], 0, 0, 0, ["$_init_59", $rt_wrapFunction1(jur_UnifiedQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_UnifiedQuantifierSet_matches), "$find", $rt_wrapFunction3(jur_UnifiedQuantifierSet_find)],
...
ju_BitSet, 0, jl_Object, [jl_Cloneable, ji_Serializable], 1, 0, 0, ["$_init_", $rt_wrapFunction0(ju_BitSet__init_0), "$_init_1", $rt_wrapFunction1(ju_BitSet__init_), "$set0", $rt_wrapFunction1(ju_BitSet_set), "$set", $rt_wrapFunction2(ju_BitSet_set0), "$clear", $rt_wrapFunction1(ju_BitSet_clear0), "$clear0", $rt_wrapFunction2(ju_BitSet_clear), "$get0", $rt_wrapFunction1(ju_BitSet_get), "$nextSetBit", $rt_wrapFunction1(ju_BitSet_nextSetBit), "$nextClearBit", $rt_wrapFunction1(ju_BitSet_nextClearBit), "$intersects",
...
jur_AbstractCharClass$LazyJavaJavaIdentifierStart$1, "AbstractCharClass$LazyJavaJavaIdentifierStart$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_4", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaJavaIdentifierStart$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaJavaIdentifierStart$1_contains)],
jur_NonCapFSet, "NonCapFSet", 2, jur_FSet, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_NonCapFSet__init_), "$matches", $rt_wrapFunction3(jur_NonCapFSet_matches), "$getName", $rt_wrapFunction0(jur_NonCapFSet_getName), "$hasConsumed", $rt_wrapFunction1(jur_NonCapFSet_hasConsumed)],
ju_Arrays, 0, jl_Object, [], 1, 0, 0, 0,
jur_CharSet, "CharSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_58", $rt_wrapFunction1(jur_CharSet__init_0), "$charCount", $rt_wrapFunction0(jur_CharSet_charCount), "$accepts", $rt_wrapFunction2(jur_CharSet_accepts), "$find", $rt_wrapFunction3(jur_CharSet_find), "$findBack", $rt_wrapFunction4(jur_CharSet_findBack), "$getName", $rt_wrapFunction0(jur_CharSet_getName), "$getChar", $rt_wrapFunction0(jur_CharSet_getChar), "$first", $rt_wrapFunction1(jur_CharSet_first)],
jur_UCISupplCharSet, "UCISupplCharSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_UCISupplCharSet__init_), "$accepts", $rt_wrapFunction2(jur_UCISupplCharSet_accepts), "$getName", $rt_wrapFunction0(jur_UCISupplCharSet_getName)],
jl_System, 0, jl_Object, [], 17, 0, 0, 0,
jur_CharClass$3, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_69", $rt_wrapFunction3(jur_CharClass$3__init_), "$contains", $rt_wrapFunction1(jur_CharClass$3_contains)],
jur_CharClass$4, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_67", $rt_wrapFunction4(jur_CharClass$4__init_), "$contains", $rt_wrapFunction1(jur_CharClass$4_contains)],
jur_CharClass$1, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_68", $rt_wrapFunction2(jur_CharClass$1__init_), "$contains", $rt_wrapFunction1(jur_CharClass$1_contains)],
jur_CharClass$2, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_69", $rt_wrapFunction3(jur_CharClass$2__init_), "$contains", $rt_wrapFunction1(jur_CharClass$2_contains)],
jur_AbstractCharClass$LazyRange, 0, jur_AbstractCharClass$LazyCharClass, [], 0, 0, 0, ["$_init_15", $rt_wrapFunction2(jur_AbstractCharClass$LazyRange__init_0), "$computeValue", $rt_wrapFunction0(jur_AbstractCharClass$LazyRange_computeValue)],
jur_CharClass$7, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_68", $rt_wrapFunction2(jur_CharClass$7__init_), "$contains", $rt_wrapFunction1(jur_CharClass$7_contains)],
...
jur_CharClass$8, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_70", $rt_wrapFunction3(jur_CharClass$8__init_), "$contains", $rt_wrapFunction1(jur_CharClass$8_contains)],
jur_CharClass$5, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_67", $rt_wrapFunction4(jur_CharClass$5__init_), "$contains", $rt_wrapFunction1(jur_CharClass$5_contains)],
jur_CharClass$6, 0, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_68", $rt_wrapFunction2(jur_CharClass$6__init_), "$contains", $rt_wrapFunction1(jur_CharClass$6_contains)],
...
jur_CICharSet, "CICharSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_58", $rt_wrapFunction1(jur_CICharSet__init_0), "$accepts", $rt_wrapFunction2(jur_CICharSet_accepts), "$getName", $rt_wrapFunction0(jur_CICharSet_getName)],
jur_SupplCharSet, "SupplCharSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_SupplCharSet__init_), "$accepts", $rt_wrapFunction2(jur_SupplCharSet_accepts), "$find", $rt_wrapFunction3(jur_SupplCharSet_find), "$findBack", $rt_wrapFunction4(jur_SupplCharSet_findBack), "$getName", $rt_wrapFunction0(jur_SupplCharSet_getName), "$getCodePoint", $rt_wrapFunction0(jur_SupplCharSet_getCodePoint), "$first", $rt_wrapFunction1(jur_SupplCharSet_first)],
jur_AbstractCharClass$LazyJavaLowerCase$1, "AbstractCharClass$LazyJavaLowerCase$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_8", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaLowerCase$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaLowerCase$1_contains)],
otrr_ReflectionInfo, 0, jl_Object, [], 1025, 0, 0, 0,
otjc_JSNumber, 0, jl_Object, [otj_JSObject], 1025, 0, 0, 0,
jur_AbstractCharClass$LazyCategoryScope, 0, jur_AbstractCharClass$LazyCharClass, [], 0, 0, 0, ["$_init_22", $rt_wrapFunction2(jur_AbstractCharClass$LazyCategoryScope__init_1), "$_init_23", $rt_wrapFunction3(jur_AbstractCharClass$LazyCategoryScope__init_0), "$computeValue", $rt_wrapFunction0(jur_AbstractCharClass$LazyCategoryScope_computeValue)],
ju_Collections$9, 0, ju_AbstractList, [], 0, 0, 0, ["$_init_73", $rt_wrapFunction1(ju_Collections$9__init_), "$get", $rt_wrapFunction1(ju_Collections$9_get), "$size", $rt_wrapFunction0(ju_Collections$9_size)],
jur_SupplRangeSet, "SupplRangeSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_42", $rt_wrapFunction1(jur_SupplRangeSet__init_), "$matches", $rt_wrapFunction3(jur_SupplRangeSet_matches), "$getName", $rt_wrapFunction0(jur_SupplRangeSet_getName), "$contains", $rt_wrapFunction1(jur_SupplRangeSet_contains), "$first", $rt_wrapFunction1(jur_SupplRangeSet_first), "$getChars", $rt_wrapFunction0(jur_SupplRangeSet_getChars), "$setNext", $rt_wrapFunction1(jur_SupplRangeSet_setNext), "$hasConsumed", $rt_wrapFunction1(jur_SupplRangeSet_hasConsumed)],
jur_UCISupplRangeSet, "UCISupplRangeSet", 2, jur_SupplRangeSet, [], 0, 0, 0, ["$_init_42", $rt_wrapFunction1(jur_UCISupplRangeSet__init_0), "$contains", $rt_wrapFunction1(jur_UCISupplRangeSet_contains), "$getName", $rt_wrapFunction0(jur_UCISupplRangeSet_getName)],
...
jur_HangulDecomposedCharSet, "HangulDecomposedCharSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_57", $rt_wrapFunction2(jur_HangulDecomposedCharSet__init_), "$setNext", $rt_wrapFunction1(jur_HangulDecomposedCharSet_setNext), "$getName", $rt_wrapFunction0(jur_HangulDecomposedCharSet_getName), "$matches", $rt_wrapFunction3(jur_HangulDecomposedCharSet_matches), "$first", $rt_wrapFunction1(jur_HangulDecomposedCharSet_first), "$hasConsumed", $rt_wrapFunction1(jur_HangulDecomposedCharSet_hasConsumed)],
...
jur_AbstractCharClass$LazyJavaTitleCase$1, "AbstractCharClass$LazyJavaTitleCase$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_37", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaTitleCase$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaTitleCase$1_contains)],
igiae_SchemaIssue, "SchemaIssue", 13, jl_Object, [], 17, 0, 0, ["$_init_21", $rt_wrapFunction3(igiae_SchemaIssue__init_), "$getPath", $rt_wrapFunction0(igiae_SchemaIssue_getPath), "$getMessage", $rt_wrapFunction0(igiae_SchemaIssue_getMessage), "$getFix", $rt_wrapFunction0(igiae_SchemaIssue_getFix), "$toString", $rt_wrapFunction0(igiae_SchemaIssue_toString)],
...
jur_AbstractCharClass$LazyJavaMirrored$1, "AbstractCharClass$LazyJavaMirrored$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_48", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaMirrored$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaMirrored$1_contains)],
otjc_JSBoolean, 0, jl_Object, [otj_JSObject], 1025, 0, 0, 0,
igiae_ActivationPlan, 0, jl_Object, [], 17, 0, 0, ["$_init_13", $rt_wrapFunction2(igiae_ActivationPlan__init_), "$getOrder", $rt_wrapFunction0(igiae_ActivationPlan_getOrder), "$getCycles", $rt_wrapFunction0(igiae_ActivationPlan_getCycles)],
ju_LinkedHashMapIterator, 0, jl_Object, [], 0, 0, 0, ["$_init_20", $rt_wrapFunction2(ju_LinkedHashMapIterator__init_), "$hasNext", $rt_wrapFunction0(ju_LinkedHashMapIterator_hasNext), "$checkConcurrentMod", $rt_wrapFunction0(ju_LinkedHashMapIterator_checkConcurrentMod), "$makeNext", $rt_wrapFunction0(ju_LinkedHashMapIterator_makeNext)],
jur_AbstractCharClass$LazyJavaISOControl$1, "AbstractCharClass$LazyJavaISOControl$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_49", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaISOControl$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaISOControl$1_contains)],
ju_LinkedHashMapKeySet, 0, ju_AbstractSet, [ju_SequencedSet], 0, 0, 0, ["$_init_20", $rt_wrapFunction2(ju_LinkedHashMapKeySet__init_), "$iterator", $rt_wrapFunction0(ju_LinkedHashMapKeySet_iterator)],
jur_WordBoundary, "WordBoundary", 2, jur_AbstractSet, [], 0, 0, 0, ["$_init_35", $rt_wrapFunction1(jur_WordBoundary__init_0), "$matches", $rt_wrapFunction3(jur_WordBoundary_matches), "$hasConsumed", $rt_wrapFunction1(jur_WordBoundary_hasConsumed), "$getName", $rt_wrapFunction0(jur_WordBoundary_getName)],
jur_UEOLSet, "UEOLSet", 2, jur_AbstractSet, [], 16, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_UEOLSet__init_), "$matches", $rt_wrapFunction3(jur_UEOLSet_matches), "$hasConsumed", $rt_wrapFunction1(jur_UEOLSet_hasConsumed), "$getName", $rt_wrapFunction0(jur_UEOLSet_getName)],
...
ju_HashMap$1, 0, ju_AbstractSet, [], 0, 0, 0, ["$_init_18", $rt_wrapFunction1(ju_HashMap$1__init_), "$iterator", $rt_wrapFunction0(ju_HashMap$1_iterator)],
jur_UCICharSet, "UCICharSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_58", $rt_wrapFunction1(jur_UCICharSet__init_), "$accepts", $rt_wrapFunction2(jur_UCICharSet_accepts), "$getName", $rt_wrapFunction0(jur_UCICharSet_getName)],
jl_Double, "Double", 3, jl_Number, [jl_Comparable], 1, 0, () => jl_Double_$callClinit(), ["$_init_44", $rt_wrapFunction1(jl_Double__init_), "$doubleValue", $rt_wrapFunction0(jl_Double_doubleValue), "$toString", $rt_wrapFunction0(jl_Double_toString0), "$equals", $rt_wrapFunction1(jl_Double_equals0), "$hashCode0", $rt_wrapFunction0(jl_Double_hashCode)],
jur_AtomicFSet, "AtomicFSet", 2, jur_FSet, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_AtomicFSet__init_), "$matches", $rt_wrapFunction3(jur_AtomicFSet_matches), "$getIndex", $rt_wrapFunction0(jur_AtomicFSet_getIndex), "$getName", $rt_wrapFunction0(jur_AtomicFSet_getName), "$hasConsumed", $rt_wrapFunction1(jur_AtomicFSet_hasConsumed)],
jur_LowSurrogateCharSet, "LowSurrogateCharSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_58", $rt_wrapFunction1(jur_LowSurrogateCharSet__init_0), "$setNext", $rt_wrapFunction1(jur_LowSurrogateCharSet_setNext), "$matches", $rt_wrapFunction3(jur_LowSurrogateCharSet_matches), "$find", $rt_wrapFunction3(jur_LowSurrogateCharSet_find), "$findBack", $rt_wrapFunction4(jur_LowSurrogateCharSet_findBack), "$getName", $rt_wrapFunction0(jur_LowSurrogateCharSet_getName), "$first", $rt_wrapFunction1(jur_LowSurrogateCharSet_first),
"$hasConsumed", $rt_wrapFunction1(jur_LowSurrogateCharSet_hasConsumed)],
jur_CompositeGroupQuantifierSet, "CompositeGroupQuantifierSet", 2, jur_GroupQuantifierSet, [], 0, 0, 0, ["$_init_45", function(var_1, var_2, var_3, var_4, var_5) { jur_CompositeGroupQuantifierSet__init_(this, var_1, var_2, var_3, var_4, var_5); }, "$matches", $rt_wrapFunction3(jur_CompositeGroupQuantifierSet_matches), "$getName", $rt_wrapFunction0(jur_CompositeGroupQuantifierSet_getName)],
jur_RelCompositeGroupQuantifierSet, "RelCompositeGroupQuantifierSet", 2, jur_CompositeGroupQuantifierSet, [], 0, 0, 0, ["$_init_45", function(var_1, var_2, var_3, var_4, var_5) { jur_RelCompositeGroupQuantifierSet__init_(this, var_1, var_2, var_3, var_4, var_5); }, "$matches", $rt_wrapFunction3(jur_RelCompositeGroupQuantifierSet_matches)],
ju_ArrayList, "ArrayList", 1, ju_AbstractList, [jl_Cloneable, ji_Serializable, ju_RandomAccess], 1, 0, 0, ["$_init_", $rt_wrapFunction0(ju_ArrayList__init_2), "$_init_1", $rt_wrapFunction1(ju_ArrayList__init_0), "$_init_72", $rt_wrapFunction1(ju_ArrayList__init_1), "$ensureCapacity", $rt_wrapFunction1(ju_ArrayList_ensureCapacity), "$get", $rt_wrapFunction1(ju_ArrayList_get), "$size", $rt_wrapFunction0(ju_ArrayList_size), "$set1", $rt_wrapFunction2(ju_ArrayList_set), "$add2", $rt_wrapFunction1(ju_ArrayList_add),
"$add1", $rt_wrapFunction2(ju_ArrayList_add0), "$remove", $rt_wrapFunction1(ju_ArrayList_remove), "$clear1", $rt_wrapFunction0(ju_ArrayList_clear), "$toString", $rt_wrapFunction0(ju_ArrayList_toString), "$hashCode0", $rt_wrapFunction0(ju_ArrayList_hashCode)],
ju_LinkedHashMapIterator$EntryIterator, 0, ju_LinkedHashMapIterator, [ju_Iterator], 0, 0, 0, ["$_init_20", $rt_wrapFunction2(ju_LinkedHashMapIterator$EntryIterator__init_), "$next0", $rt_wrapFunction0(ju_LinkedHashMapIterator$EntryIterator_next), "$next", $rt_wrapFunction0(ju_LinkedHashMapIterator$EntryIterator_next0)],
jur_RelAltGroupQuantifierSet, "RelAltGroupQuantifierSet", 2, jur_AltGroupQuantifierSet, [], 0, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_RelAltGroupQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_RelAltGroupQuantifierSet_matches)],
jur_IntHash, 0, jl_Object, [], 0, 0, 0, 0]);
$rt_metadata([jl_String, "String", 3, jl_Object, [ji_Serializable, jl_Comparable, jl_CharSequence], 17, 0, () => jl_String_$callClinit(), ["$_init_", $rt_wrapFunction0(jl_String__init_2), "$_init_29", $rt_wrapFunction1(jl_String__init_0), "$_init_46", $rt_wrapFunction1(jl_String__init_4), "$_init_16", $rt_wrapFunction3(jl_String__init_5), "$charAt", $rt_wrapFunction1(jl_String_charAt), "$length", $rt_wrapFunction0(jl_String_length), "$isEmpty", $rt_wrapFunction0(jl_String_isEmpty), "$startsWith1", $rt_wrapFunction2(jl_String_startsWith0),
"$startsWith", $rt_wrapFunction1(jl_String_startsWith), "$indexOf2", $rt_wrapFunction2(jl_String_indexOf), "$indexOf", $rt_wrapFunction1(jl_String_indexOf1), "$lastIndexOf2", $rt_wrapFunction2(jl_String_lastIndexOf), "$lastIndexOf", $rt_wrapFunction1(jl_String_lastIndexOf1), "$indexOf1", $rt_wrapFunction2(jl_String_indexOf0), "$lastIndexOf1", $rt_wrapFunction2(jl_String_lastIndexOf0), "$substring0", $rt_wrapFunction2(jl_String_substring), "$substring", $rt_wrapFunction1(jl_String_substring0), "$subSequence",
$rt_wrapFunction2(jl_String_subSequence), "$contains1", $rt_wrapFunction1(jl_String_contains), "$toString", $rt_wrapFunction0(jl_String_toString), "$toCharArray", $rt_wrapFunction0(jl_String_toCharArray), "$equals", $rt_wrapFunction1(jl_String_equals), "$hashCode0", $rt_wrapFunction0(jl_String_hashCode), "$split", $rt_wrapFunction1(jl_String_split)],
ju_LinkedHashSet, "LinkedHashSet", 1, ju_HashSet, [ju_SequencedSet, jl_Cloneable, ji_Serializable], 1, 0, 0, ["$_init_", $rt_wrapFunction0(ju_LinkedHashSet__init_)],
jur_ReluctantAltQuantifierSet, "ReluctantAltQuantifierSet", 2, jur_AltQuantifierSet, [], 0, 0, 0, ["$_init_11", $rt_wrapFunction3(jur_ReluctantAltQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_ReluctantAltQuantifierSet_matches)],
...
"$getFix", $rt_wrapFunction0(igiae_JsonSchema_getFix), "$setFix", $rt_wrapFunction1(igiae_JsonSchema_setFix)],
...
jl_UnsupportedOperationException, 0, jl_RuntimeException, [], 1, 0, 0, ["$_init_", $rt_wrapFunction0(jl_UnsupportedOperationException__init_)],
...
jl_IllegalArgumentException, 0, jl_RuntimeException, [], 1, 0, 0, ["$_init_", $rt_wrapFunction0(jl_IllegalArgumentException__init_0), "$_init_0", $rt_wrapFunction1(jl_IllegalArgumentException__init_1)],
jl_NumberFormatException, 0, jl_IllegalArgumentException, [], 1, 0, 0, ["$_init_", $rt_wrapFunction0(jl_NumberFormatException__init_1), "$_init_0", $rt_wrapFunction1(jl_NumberFormatException__init_)],
jur_PosCompositeGroupQuantifierSet, "PosCompositeGroupQuantifierSet", 2, jur_CompositeGroupQuantifierSet, [], 0, 0, 0, ["$_init_45", function(var_1, var_2, var_3, var_4, var_5) { jur_PosCompositeGroupQuantifierSet__init_(this, var_1, var_2, var_3, var_4, var_5); }, "$matches", $rt_wrapFunction3(jur_PosCompositeGroupQuantifierSet_matches)],
jur_MultiLineEOLSet, "MultiLineEOLSet", 2, jur_AbstractSet, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_MultiLineEOLSet__init_), "$matches", $rt_wrapFunction3(jur_MultiLineEOLSet_matches), "$hasConsumed", $rt_wrapFunction1(jur_MultiLineEOLSet_hasConsumed), "$getName", $rt_wrapFunction0(jur_MultiLineEOLSet_getName)],
...
jur_AbstractCharClass$LazyJavaDigit$1, "AbstractCharClass$LazyJavaDigit$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_28", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaDigit$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaDigit$1_contains)],
...
jur_HighSurrogateCharSet, "HighSurrogateCharSet", 2, jur_JointSet, [], 0, 0, 0, ["$_init_58", $rt_wrapFunction1(jur_HighSurrogateCharSet__init_), "$setNext", $rt_wrapFunction1(jur_HighSurrogateCharSet_setNext), "$matches", $rt_wrapFunction3(jur_HighSurrogateCharSet_matches), "$find", $rt_wrapFunction3(jur_HighSurrogateCharSet_find), "$findBack", $rt_wrapFunction4(jur_HighSurrogateCharSet_findBack), "$getName", $rt_wrapFunction0(jur_HighSurrogateCharSet_getName), "$first", $rt_wrapFunction1(jur_HighSurrogateCharSet_first),
"$hasConsumed", $rt_wrapFunction1(jur_HighSurrogateCharSet_hasConsumed)],
jur_ReluctantCompositeQuantifierSet, "ReluctantCompositeQuantifierSet", 2, jur_CompositeQuantifierSet, [], 0, 0, 0, ["$_init_12", $rt_wrapFunction4(jur_ReluctantCompositeQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_ReluctantCompositeQuantifierSet_matches)],
jl_NullPointerException, 0, jl_RuntimeException, [], 1, 0, 0, ["$_init_0", $rt_wrapFunction1(jl_NullPointerException__init_1), "$_init_", $rt_wrapFunction0(jl_NullPointerException__init_0)],
...
jur_AbstractCharClass$LazyJavaSpaceChar$1, "AbstractCharClass$LazyJavaSpaceChar$1", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_32", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaSpaceChar$1__init_), "$contains", $rt_wrapFunction1(jur_AbstractCharClass$LazyJavaSpaceChar$1_contains)],
ju_LinkedHashMapEntrySet, 0, ju_AbstractSet, [ju_SequencedSet], 0, 0, 0, ["$_init_20", $rt_wrapFunction2(ju_LinkedHashMapEntrySet__init_), "$iterator", $rt_wrapFunction0(ju_LinkedHashMapEntrySet_iterator)],
...
jur_PatternSyntaxException, 0, jl_IllegalArgumentException, [], 1, 0, 0, ["$_init_54", $rt_wrapFunction3(jur_PatternSyntaxException__init_0)],
...
jur_Pattern, 0, jl_Object, [ji_Serializable], 17, 0, 0, ["$matcher", $rt_wrapFunction1(jur_Pattern_matcher), "$split1", $rt_wrapFunction2(jur_Pattern_split0), "$split0", $rt_wrapFunction1(jur_Pattern_split), "$pattern", $rt_wrapFunction0(jur_Pattern_pattern), "$namedGroups", $rt_wrapFunction0(jur_Pattern_namedGroups), "$groupCount", $rt_wrapFunction0(jur_Pattern_groupCount), "$compCount", $rt_wrapFunction0(jur_Pattern_compCount), "$consCount", $rt_wrapFunction0(jur_Pattern_consCount)],
jur_PosAltGroupQuantifierSet, "PosAltGroupQuantifierSet", 2, jur_AltGroupQuantifierSet, [], 0, 0, 0, ["$_init_10", $rt_wrapFunction3(jur_PosAltGroupQuantifierSet__init_), "$matches", $rt_wrapFunction3(jur_PosAltGroupQuantifierSet_matches), "$setNext", $rt_wrapFunction1(jur_PosAltGroupQuantifierSet_setNext)],
ju_Collections$13, 0, ju_AbstractMap, [], 0, 0, 0, ["$_init_14", $rt_wrapFunction1(ju_Collections$13__init_)],
...
jur_UMultiLineEOLSet, "UMultiLineEOLSet", 2, jur_AbstractSet, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_UMultiLineEOLSet__init_), "$matches", $rt_wrapFunction3(jur_UMultiLineEOLSet_matches), "$hasConsumed", $rt_wrapFunction1(jur_UMultiLineEOLSet_hasConsumed), "$getName", $rt_wrapFunction0(jur_UMultiLineEOLSet_getName)],
ju_LinkedHashMapIterator$KeyIterator, 0, ju_LinkedHashMapIterator, [ju_Iterator], 0, 0, 0, ["$_init_20", $rt_wrapFunction2(ju_LinkedHashMapIterator$KeyIterator__init_), "$next", $rt_wrapFunction0(ju_LinkedHashMapIterator$KeyIterator_next)],
...
otciu_UnicodeHelper$Range, "UnicodeHelper$Range", 8, jl_Object, [], 1, 0, 0, ["$_init_31", $rt_wrapFunction3(otciu_UnicodeHelper$Range__init_)],
otcit_DoubleAnalyzer, 0, jl_Object, [], 17, 0, () => otcit_DoubleAnalyzer_$callClinit(), 0,
jur_EOLSet, "EOLSet", 2, jur_AbstractSet, [], 16, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_EOLSet__init_), "$matches", $rt_wrapFunction3(jur_EOLSet_matches), "$hasConsumed", $rt_wrapFunction1(jur_EOLSet_hasConsumed), "$getName", $rt_wrapFunction0(jur_EOLSet_getName)],
...
otciu_CharMapping, 0, jl_Object, [], 1, 0, 0, ["$_init_30", $rt_wrapFunction2(otciu_CharMapping__init_)],
...
igiaj_EngineJs$1, 0, jl_Object, [igiae_Diagnostics$Sink], 0, 0, 0, ["$_init_7", $rt_wrapFunction1(igiaj_EngineJs$1__init_), "$accept", $rt_wrapFunction1(igiaj_EngineJs$1_accept)],
jur_Lexer, 0, jl_Object, [], 0, 0, 0, ["$_init_53", $rt_wrapFunction2(jur_Lexer__init_), "$peek", $rt_wrapFunction0(jur_Lexer_peek), "$setMode", $rt_wrapFunction1(jur_Lexer_setMode), "$restoreFlags", $rt_wrapFunction1(jur_Lexer_restoreFlags), "$peekSpecial", $rt_wrapFunction0(jur_Lexer_peekSpecial), "$isSpecial", $rt_wrapFunction0(jur_Lexer_isSpecial), "$isNextSpecial", $rt_wrapFunction0(jur_Lexer_isNextSpecial), "$next1", $rt_wrapFunction0(jur_Lexer_next), "$nextSpecial", $rt_wrapFunction0(jur_Lexer_nextSpecial),
"$lookAhead", $rt_wrapFunction0(jur_Lexer_lookAhead), "$back", $rt_wrapFunction0(jur_Lexer_back), "$toString", $rt_wrapFunction0(jur_Lexer_toString), "$isEmpty", $rt_wrapFunction0(jur_Lexer_isEmpty), "$isLetter1", $rt_wrapFunction0(jur_Lexer_isLetter0), "$isHighSurrogate0", $rt_wrapFunction0(jur_Lexer_isHighSurrogate0), "$isLowSurrogate0", $rt_wrapFunction0(jur_Lexer_isLowSurrogate0), "$getIndex", $rt_wrapFunction0(jur_Lexer_getIndex)],
...
otci_CharFlow, 0, jl_Object, [], 1, 0, 0, ["$_init_29", $rt_wrapFunction1(otci_CharFlow__init_)],
igiae_ManifestSchema, 0, jl_Object, [], 17, 0, 0, 0]);
$rt_metadata([jur_RangeSet, "RangeSet", 2, jur_LeafSet, [], 0, 0, 0, ["$_init_42", $rt_wrapFunction1(jur_RangeSet__init_), "$accepts", $rt_wrapFunction2(jur_RangeSet_accepts), "$getName", $rt_wrapFunction0(jur_RangeSet_getName), "$first", $rt_wrapFunction1(jur_RangeSet_first), "$getChars", $rt_wrapFunction0(jur_RangeSet_getChars)],
jur_UnicodeCategory, "UnicodeCategory", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_UnicodeCategory__init_), "$contains", $rt_wrapFunction1(jur_UnicodeCategory_contains)],
jur_UnicodeCategoryScope, "UnicodeCategoryScope", 2, jur_UnicodeCategory, [], 0, 0, 0, ["$_init_1", $rt_wrapFunction1(jur_UnicodeCategoryScope__init_), "$contains", $rt_wrapFunction1(jur_UnicodeCategoryScope_contains)],
jur_CharClass, "CharClass", 2, jur_AbstractCharClass, [], 0, 0, 0, ["$_init_", $rt_wrapFunction0(jur_CharClass__init_2), "$_init_55", $rt_wrapFunction2(jur_CharClass__init_0), "$_init_62", $rt_wrapFunction3(jur_CharClass__init_1), "$add", $rt_wrapFunction1(jur_CharClass_add), "$add3", $rt_wrapFunction1(jur_CharClass_add1), "$add0", $rt_wrapFunction2(jur_CharClass_add0), "$union", $rt_wrapFunction1(jur_CharClass_union), "$intersection", $rt_wrapFunction1(jur_CharClass_intersection), "$contains", $rt_wrapFunction1(jur_CharClass_contains),
...
jur_UCIDecomposedCharSet, "UCIDecomposedCharSet", 2, jur_DecomposedCharSet, [], 0, 0, 0, ["$_init_25", $rt_wrapFunction2(jur_UCIDecomposedCharSet__init_)],
otrr_ClassInfo, 0, otrr_ReflectionInfo, [], 17, 0, 0, ["$newArrayInstance", $rt_wrapFunction1(otrr_ClassInfo_newArrayInstance)],
ju_Arrays$ArrayAsList, 0, ju_AbstractList, [ju_RandomAccess, ji_Serializable], 0, 0, 0, ["$_init_39", $rt_wrapFunction1(ju_Arrays$ArrayAsList__init_), "$get", $rt_wrapFunction1(ju_Arrays$ArrayAsList_get), "$size", $rt_wrapFunction0(ju_Arrays$ArrayAsList_size)],
...
$rt_stringPool(["String is null", "String is empty", "String contains invalid digits: ", "String contains digits out of radix ", ": ", "The value is too big for int type: ", "The value is too big for integer type", "Illegal radix: ", "JointSet", "CI back reference: ", "id", "services", "provides", "consumes", "CI sequence: ", "<Quant>", "<GroupQuant>", "UCI back reference: ", "UCI sequence: ", "<DotAllQuant>", "fSet", "BehindFSet", "null", "CompositeRangeSet:  <nonsurrogate> ", " <surrogate> ", "1", "INTISY_PLUGIN_STRICT",
"ignored unknown ", " \"", "\" from ", "[plugin-api] ", " ", "^ ", "range:", "FinalSet", "(this Map)", ", ", "<Empty set>", "", "Lower", "Upper", "ASCII", "Alpha", "Digit", "Alnum", "Punct", "Graph", "Print", "Blank", "Cntrl", "XDigit", "javaLowerCase", "javaUpperCase", "javaWhitespace", "javaMirrored", "javaDefined", "javaDigit", "javaIdentifierIgnorable", "javaISOControl", "javaJavaIdentifierPart", "javaJavaIdentifierStart", "javaLetter", "javaLetterOrDigit", "javaSpaceChar", "javaTitleCase", "javaUnicodeIdentifierPart",
...
"AheadFSet", "NonCapJointSet", "PosLookaheadJointSet", "NegLookaheadJointSet", "[", "] ", "\n  fix: ", "(this Collection)", "0", "the manifest", "(root)", "capabilities", "services.provides", "services.consumes", "permissions", "(unknown plugin)", "plugin.json ", "entry", "capabilities are declared but no entry names the module that provides them", "add \"entry\": \"dist/index.js\"", "\"", "\" is not a path inside the repo", "use a repo-relative path with no leading slash and no ..", "/", "\\", "[\\\\/]", "..",
":", "services.provides[", "]", "\" is neither namespaced by this plugin nor a well-known service id", "rename it to \"", "\", or use one of: ", "\" is listed twice", "remove the duplicate entry", "PosBehindJointSet", "sequence: ", "EOI", "UCI range:", "\n", "false", "true", "^", "NegBehindJointSet", "back reference: ", "<DotQuant>", "PreviousMatch", "[L", "NonCapFSet", "UCI ", "Either src or dest is null", "string", "number", "boolean", ".", "DotAll", "CI ", "decomposed Hangul syllable:", " (fix: ", ")", "WordBoundary",
"<EOL>", "object", "array", "integer", "required field \"", "\" is missing", "add \"", "\" to ", "field", "\" does not match ", "make ", " match ", "\" is not one of ", "set ", " to one of ", "expected a value >= ", ", got ", " to a value >= ", "expected ", " to a ", "AtomicFSet", "[]", "posFSet", "<MultiLine $>", "<SOL>", "Patter is null", "\\Q", "\\E", "\\\\E\\Q", "<Unix MultiLine $>", "Name capturing group should start with letter", "Is", "In", "$schema", "Pointer at the published manifest schema, for an editor\'s completion and validation.",
"The plugin\'s permanent identity, matching its repository name.", "^[a-z0-9]+(-[a-z0-9]+)*$", "use lowercase words joined by single hyphens, for example \"config-ledger\"", "The lowest API major version this plugin needs. A floor, not a build tag.", "set \"api\" to the lowest API major version this plugin needs, for example 1", "api", "The built module a host imports. Required once capabilities are declared.", "point \"entry\" at the built module a host imports, for example \"dist/index.js\"", "displayName",
"The name a surface shows instead of the id.", "icon", "Path to a square-viewBox SVG mark, relative to the repo root.", "Host-facing abilities this plugin provides at activation.", "list capability ids as strings, for example [\"provider\", \"screens\"]", "Declared permissions, surfaced at install and in dashboards.", "lifecycle", "publish", "repo", "The single machine-readable description of a repo in the intisy-ai ecosystem.", "intisy-ai plugin manifest", "Service ids this plugin registers, each namespaced by its own id or a well-known bare id.",
"Service ids this plugin asks for.", "The inter-plugin contract: what this plugin offers other plugins, and what it asks of them.", "install", "The entry exports install(ctx), run once after first deploy.", "repair", "The entry exports repair(ctx), run on demand from a host.", "Which optional lifecycle hooks the entry module exports.", "scopedOnly", "Publish only as @intisy-ai/<name>, because the unscoped name is unavailable.", "How the repo is published to npm.", "role", "The role phrase, capitalized, without the fixed \"for the intisy-ai AI-proxy ecosystem.\" suffix.",
"category", "The single category topic, for example core-library or ai-provider.", "Domain topics, for example claude or gemini.", "domains", "tech", "The primary tech topic, typescript or java.", "Repository metadata: the GitHub description and topic set are derived from it."]);
...
export { igiaj_EngineJs_activationOrder$exported$0 as activationOrder, igiaj_EngineJs_assertManifest$exported$1 as assertManifest, igiaj_EngineJs_pluginError$exported$2 as pluginError, igiaj_EngineJs_isPluginError$exported$3 as isPluginError, igiaj_EngineJs_setStrict$exported$4 as setStrict, igiaj_EngineJs_setDiagnosticSink$exported$5 as setDiagnosticSink };
