...
import { pluginError, setDiagnosticSink as setEngineDiagnosticSink } from "../../generated/engine.js";
import type { ContextSurface, PluginRuntimeShape } from "../../generated/engine.js";
import type { Plugin, PluginManifest } from "../../generated/api.js";
...
describe("startPlugins", () => {
  ...
  // How a plugin gets behaviour belonging to a library it may not link: the host links it once and
  // offers a handle, so the plugin carries no private copy of it.
  it("offers a host's own service to a plugin during its activation", async () => {
    const support = { build: (name: string) => `built ${name}` };
    let seen: string | undefined;
    const scan = scanOf({
      manifest: manifest("consumer", { capabilities: [], services: { consumes: ["provider-support"] } }),
      module: {
        default: {
          activate: (ctx: ContextSurface) => {
            seen = (ctx.services.get("provider-support") as typeof support).build("lane");
          },
          deactivate: () => {},
        },
      },
    ...
    const loaded = await startPlugins(options(scan, { services: [{ id: "provider-support", implementation: support }] }));

    expect(loaded.started).toEqual(["consumer"]);
    expect(seen).toBe("built lane");
  });
...
describe("ledgerRows", () => {
  ...
  it("lists a consumed service as unresolved if its provider was quarantined or stopped", async () => {
    ...
    const loaded = await startPlugins(options(scan));
    await loaded.host.markBroken("provider", (await import("../../generated/engine.js")).pluginError("provider", "stopped", "restart it"));
